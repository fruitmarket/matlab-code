function [L_Extra,L_Ratio,IsolationDist,Dists,df,L_Intra,C] = ClusterSeparation_lratio(f,filename,ChannelValidity,iClust,varargin)
...
MClustResetGlobals('initialize'); % initialize all global variables. if not calculation of l-ratio is wrong.
...
%             FeaturesToGet = {'Energy'};
		...
		if isempty(n_records)
			n_records = MClust_CountSpikes_lratio(TTFileName);
		end
