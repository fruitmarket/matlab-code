function plot_Track_multi_cscTrace_v2
rtDir = 'E:\Dropbox\SNL\P2_Track';
load('E:\Dropbox\SNL\P2_Track\cscList_ori50hz_180926.mat');
nFile = length(T.path);
...
for iFile = 1:nFile
    ...
    hText = axes('Position',axpt(1,1,1,1,axpt(nCol,nRow,1,1:2,figSize,midInterval),wideInterval));
        text(-0.1,1.1, T.path{iFile}, 'FontSize', fontM, 'Interpreter', 'none', 'fontWeight', 'bold');
            ...
            if iT == 1
                for iCycle = 1:10
                    ...
                    if regexp(pwd,'Run')
                        ...
                        pw(iCycle) = patch([300*iCycle-300, 300*iCycle,300*iCycle,300*iCycle-300],[10 10 15 15],colorBlueGrad{iCycle},'lineStyle','none');
                    end
                end
            ...
            if iT == 1
                for iCycle = 1:10
                    ...
                    if regexp(pwd,'Run')
                        ...
                        pw(iCycle) = patch([300*iCycle-3300, 300*iCycle-3000,300*iCycle-3000,300*iCycle-3300],[10 10 15 15],colorBlueGrad{11-iCycle},'lineStyle','none');
                    end
                end
    ...
    cd('E:\Dropbox\SNL\P2_Track\example_track50hz_csc');
    if regexp(T.path{iFile},'Run')
        print('-painters','-r300','-dtiff',['plot_csc_',num2str(T.cscID(iFile)),'_Run','.tif']);
        print('-painters','-r300','-depsc',['plot_csc_',num2str(T.cscID(iFile)),'_Run','.ai']);
    else
        print('-painters','-r300','-dtiff',['plot_csc_',num2str(T.cscID(iFile)),'_Rw','.tif']);
        print('-painters','-r300','-depsc',['plot_csc_',num2str(T.cscID(iFile)),'_Rw','.ai']);
    end
    close;
end