function x = meanFR(time1, time2, spikeData)
%%%%%%
...
narginchk(3, 3);
nTime = length(time1);
meanFiringRate = zeros(nTime,1);

if length(time1) ~= length(time2)
    disp('Different time array! Time array should be same size.');
else
    for iTime = 1:nTime
        meanFiringRate(iTime,1) = sum(histc(spikeData,[time1(iTime), time2(iTime)]))/(diff([time1(iTime),time2(iTime)])/1000);
    end
    x = meanFiringRate;
end
end