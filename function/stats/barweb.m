...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
    ...
    % handles.bars = bar(barvalues, width);
    % handles.bars = bar(barvalues, width,'edgecolor','none');
	...
	if strcmp(legend_type, 'axis')
		for i = 1:numbars
			...
			for j = 1:length(xdata)
				text(xdata(j),  -0.03*ymax*1.1, bw_legend(i), 'Rotation', 60, 'fontsize', 16, 'HorizontalAlignment', 'right');
			end
		end
	...
	if ~isempty(bw_title)
		title(bw_title, 'fontsize',20);
	end
	if ~isempty(bw_xlabel)
		xlabel(bw_xlabel, 'fontsize',16);
	end
	if ~isempty(bw_ylabel)
		ylabel(bw_ylabel, 'fontsize',16);
	end
	
	set(gca, 'xticklabel', groupnames, 'box', 'off', 'ticklength', [0 0], 'fontsize', 16, 'fontweight', 'bold', 'xtick',1:numgroups, 'linewidth', 2,'xgrid','off','ygrid','off');
