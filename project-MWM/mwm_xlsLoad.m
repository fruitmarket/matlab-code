...
for ifile =1:nfile
    ...
    latency(:,ifile) = all_info.LatencyToTarget_Seconds_(1:trial);     % 63
    distance(:,ifile) = all_info.DistanceToTarget(1:trial);
    meandistance(:,ifile) = all_info.MeanDistanceToTarget(1:trial);    % 68
    totaldistance(:,ifile) = all_info.TotalDistance(1:trial);          % 97
    speed(:,ifile) = all_info.MeanSpeedInZone_Total(1:trial);          % 160   
end
