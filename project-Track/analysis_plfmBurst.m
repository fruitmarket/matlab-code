function analysis_plfmBurst

winLight = [0 20];
winBurst = [-50 120];
binSize = 2;
resolution = 10; % (resolution * binSize = sigma (20ms);
...
% mean firing rate
    meanFR_PRE = sum(histc(spkData,time_PRE))/(diff(time_PRE)/1000);
    meanFR_STIM = sum(histc(spkData,time_STIM))/(diff(time_STIM)/1000);
    meanFR_POST = sum(histc(spkData,time_POST))/(diff(time_POST)/1000);    

% PETH & Raster plot
    spikeT = spikeWin(spkData,lightT(1:nPulse:nLight),winBurst);
    [xptLight, yptLight, pethtimeLight, pethLight, ~, ~] = rasterPETH(spikeT,true(size(lightT(1:nPulse:nLight))),winBurst,binSize,resolution,1);
    
% # of evoked spikes
    spikeT = spikeWin(spkData,lightT,winLight);
    evoSpike = cellfun(@length,spikeT);
    sum_re_evoSpike = sum(reshape(evoSpike,[nPulse, nBurst]),2)';
    
% # of burst observed
    re_evoSpike = reshape(evoSpike,[nPulse,nBurst])';
    n_evoBurst = sum(double(sum(re_evoSpike,2)>=3));
    ratio_burst = n_evoBurst/nBurst*100;
    
    save([cellName,'.mat'],...
        'meanFR_PRE','meanFR_STIM','meanFR_POST',...
        'xptLight','yptLight','pethtimeLight','pethLight',...
        're_evoSpike','sum_re_evoSpike','n_evoBurst','ratio_burst');
end
disp('### analysis_plfmBurst is done! ###');
