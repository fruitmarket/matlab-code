...
%% No-convolution, Lap-by-lap ratemap
    ...
    [~, ~, ~, pethLapRawSpatial, ~, ~] = spatialrasterPETH(spikePosition, trialIndexLap, numOccuLap_cali, winSpace, binSize, resolution, dot);
    rateMapRaw_eachLap = pethLapRawSpatial;
    save([cellName,'.mat'],'rateMapRaw_eachLap','-append');
...
%% No-convolution, Baseline ratemap (10 from middle of each block)
    ...
    [~,~,~,pethLapRawBase10_PRE,~,~] = spatialrasterPETH(spikePositionBasePRE,true(10,1),sum(numOccuLap_cali(11:20,:),1),winSpace, binSize, resolution, dot);
    [~,~,~,pethLapRawBase10_STIM,~,~] = spatialrasterPETH(spikePositionBaseSTIM,true(10,1),sum(numOccuLap_cali(41:50,:),1),winSpace, binSize, resolution, dot);
    [~,~,~,pethLapRawBase10_POST,~,~] = spatialrasterPETH(spikePositionBasePOST,true(10,1),sum(numOccuLap_cali(71:80,:),1),winSpace, binSize, resolution, dot);
    ...
    save([cellName,'.mat'],'rateMapRaw10_PRE','rateMapRaw10_STIM','rateMapRaw10_POST','rCorrRawLap_basePRE','rCorrRawLap_baseSTIM','rCorrRawLap_basePOST','-append');
