...
%% spatial correlation 1D
    ...
    [~, ~, ~, ~, pethconvSpatialPRE, ~] = spatialrasterPETH(spikePositionPRE, trialIndexPRE, numOccu_cali_PRE, winSpace, binSize, resolution, dot);
    pv_rateMap1D_PRE1 = pethconvSpatialPRE(1,:); % lap 1~15
    pv_rateMap1D_PRE2 = pethconvSpatialPRE(2,:); % lap 6~30
    
    spikePositionPOST = spikePosition(61:90);
    trialIndexspikePositionPOST = logical([repmat(A,15,1);repmat(B,15,1)]);
    [~, ~, ~, ~, pethconvSpatialPOST, ~] = spatialrasterPETH(spikePositionPRE, trialIndexspikePositionPOST, numOccu_cali_PRE, winSpace, binSize, resolution, dot);
    pv_rateMap1D_POST1 = pethconvSpatialPOST(1,:); % lap 61~75
    pv_rateMap1D_POST2 = pethconvSpatialPOST(2,:); % lap 76~90
