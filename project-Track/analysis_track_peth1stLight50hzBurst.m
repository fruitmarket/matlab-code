function analysis_track_peth1stLight50hzBurst
...
lapLightIdx = [1;(find(diff(lightTime)>lapTimeCri)+1)]; % find start light of each lap
minPulseTrack50hz = min(diff(lapLightIdx));
maxPulseTrack50hz = max(diff(lapLightIdx));
...
lightOnStm = lightTime([1; (find(diff(lightTime)>lapTimeCri)+1)]); % light interval criteria: 50ms
lightOnPre = psdlightPre([1; (find(diff(psdlightPre)>lapTimeCri)+1)]);
lightOnPost = psdlightPost([1; (find(diff(psdlightPost)>lapTimeCri)+1)]);
...
for iCell = 1:nCell
    ...
    save([cellName, '.mat'],...
        'minPulseTrack50hz', 'maxPulseTrack50hz','nLight','nBurst','nPulse',...
