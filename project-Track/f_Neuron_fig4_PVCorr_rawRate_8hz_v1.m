...
tPC_Run_PRE = cell2mat(T.rateMapRaw_PRE(tPC_DRun));
tPC_Run_STM = cell2mat(T.rateMapRaw_STIM(tPC_DRun));
tPC_Run_POST = cell2mat(T.rateMapRaw_POST(tPC_DRun));
...
inZone_PRE_Run = tPC_Run_PRE(:,lightLoc_Run(1):lightLoc_Run(2));
inZone_STM_Run = tPC_Run_STM(:,lightLoc_Run(1):lightLoc_Run(2));
inZone_POST_Run = tPC_Run_POST(:,lightLoc_Run(1):lightLoc_Run(2));

outZone_PRE_Run = tPC_Run_PRE(:,[1:lightLoc_Run(1)-1,lightLoc_Run(2)+1:nBin_50hz]);
outZone_STM_Run = tPC_Run_STM(:,[1:lightLoc_Run(1)-1,lightLoc_Run(2)+1:nBin_50hz]);
outZone_POST_Run = tPC_Run_POST(:,[1:lightLoc_Run(1)-1,lightLoc_Run(2)+1:nBin_50hz]);
...
nBin_inRun = size(inZone_PRE_Run,2);
nBin_outRun = size(outZone_PRE_Run,2);

for iCol = 1:nBin_inRun
    pvCorr_inRun_preXstm(iCol) = corr(inZone_PRE_Run(:,iCol),inZone_STM_Run(:,iCol),'rows','complete','type','Pearson');
    pvCorr_inRun_preXpost(iCol) = corr(inZone_PRE_Run(:,iCol),inZone_POST_Run(:,iCol),'rows','complete','type','Pearson');
    pvCorr_inRun_stmXpost(iCol) = corr(inZone_STM_Run(:,iCol),inZone_POST_Run(:,iCol),'rows','complete','type','Pearson');
end

for iCol = 1:nBin_outRun
    pvCorr_outRun_preXstm(iCol) = corr(outZone_PRE_Run(:,iCol),outZone_STM_Run(:,iCol),'rows','complete','type','Pearson');
    pvCorr_outRun_preXpost(iCol) = corr(outZone_PRE_Run(:,iCol),outZone_POST_Run(:,iCol),'rows','complete','type','Pearson');
    pvCorr_outRun_stmXpost(iCol) = corr(outZone_STM_Run(:,iCol),outZone_POST_Run(:,iCol),'rows','complete','type','Pearson');
end
...
% corr of total neurons
tPC_Rw_PRE = cell2mat(T.rateMapRaw_PRE(tPC_DRw));
tPC_Rw_STM = cell2mat(T.rateMapRaw_STIM(tPC_DRw));
tPC_Rw_POST = cell2mat(T.rateMapRaw_POST(tPC_DRw));
...
inZone_PRE_Rw = tPC_Rw_PRE(:,lightLoc_Rw(1):lightLoc_Rw(2));
inZone_STM_Rw = tPC_Rw_STM(:,lightLoc_Rw(1):lightLoc_Rw(2));
inZone_POST_Rw = tPC_Rw_POST(:,lightLoc_Rw(1):lightLoc_Rw(2));

outZone_PRE_Rw = tPC_Rw_PRE(:,[1:lightLoc_Rw(1)-1,lightLoc_Rw(2)+1:nBin_50hz]);
outZone_STM_Rw = tPC_Rw_STM(:,[1:lightLoc_Rw(1)-1,lightLoc_Rw(2)+1:nBin_50hz]);
outZone_POST_Rw = tPC_Rw_POST(:,[1:lightLoc_Rw(1)-1,lightLoc_Rw(2)+1:nBin_50hz]);
...
nBin_inRw = size(inZone_PRE_Rw,2);
nBin_outRw = size(outZone_PRE_Rw,2);

for iCol = 1:nBin_inRw
    pvCorr_inRw_preXstm(iCol) = corr(inZone_PRE_Rw(:,iCol),inZone_STM_Rw(:,iCol),'rows','complete','type','Pearson');
    pvCorr_inRw_preXpost(iCol) = corr(inZone_PRE_Rw(:,iCol),inZone_POST_Rw(:,iCol),'rows','complete','type','Pearson');
    pvCorr_inRw_stmXpost(iCol) = corr(inZone_STM_Rw(:,iCol),inZone_POST_Rw(:,iCol),'rows','complete','type','Pearson');
end

for iCol = 1:nBin_outRw
    pvCorr_outRw_preXstm(iCol) = corr(outZone_PRE_Rw(:,iCol),outZone_STM_Rw(:,iCol),'rows','complete','type','Pearson');
    pvCorr_outRw_preXpost(iCol) = corr(outZone_PRE_Rw(:,iCol),outZone_POST_Rw(:,iCol),'rows','complete','type','Pearson');
    pvCorr_outRw_stmXpost(iCol) = corr(outZone_STM_Rw(:,iCol),outZone_POST_Rw(:,iCol),'rows','complete','type','Pearson');
end
...
noRun_PRE = cell2mat(T.rateMapRaw_PRE(tPC_noRun));
noRun_STM = cell2mat(T.rateMapRaw_STIM(tPC_noRun));
noRun_POST = cell2mat(T.rateMapRaw_POST(tPC_noRun));
...
noRw_PRE = cell2mat(T.rateMapRaw_PRE(tPC_noRw));
noRw_STM = cell2mat(T.rateMapRaw_STIM(tPC_noRw));
noRw_POST = cell2mat(T.rateMapRaw_POST(tPC_noRw));
...
%% statistic
group_Run = [ones(nBin_inRun,1);2*ones(nBin_outRun,1);3*ones(nBin_ctrl,1)];
group_Rw = [ones(nBin_inRw,1);2*ones(nBin_outRw,1);3*ones(nBin_ctrl,1)];

% mean & sem
m_pvCorr_inlight_Run = [mean(pvCorr_inRun_preXstm), mean(pvCorr_inRun_preXpost), mean(pvCorr_inRun_stmXpost)];
sem_pvCorr_inlight_Run = [std(pvCorr_inRun_preXstm,0,2)/sqrt(nBin_inRun), std(pvCorr_inRun_preXpost,0,2)/sqrt(nBin_inRun), std(pvCorr_inRun_stmXpost,0,2)/sqrt(nBin_inRun)];

m_pvCorr_outlight_Run = [mean(pvCorr_outRun_preXstm), mean(pvCorr_outRun_preXpost), mean(pvCorr_outRun_stmXpost)];
sem_pvCorr_outlight_Run = [std(pvCorr_outRun_preXstm,0,2)/sqrt(nBin_outRun), std(pvCorr_outRun_preXpost,0,2)/sqrt(nBin_outRun), std(pvCorr_outRun_stmXpost,0,2)/sqrt(nBin_outRun)];

m_pvCorr_inlight_Rw = [mean(pvCorr_inRw_preXstm), mean(pvCorr_inRw_preXpost), mean(pvCorr_inRw_stmXpost)];
sem_pvCorr_inlight_Rw = [std(pvCorr_inRw_preXstm,0,2)/sqrt(nBin_inRw), std(pvCorr_inRw_preXpost,0,2)/sqrt(nBin_inRw), std(pvCorr_inRw_stmXpost,0,2)/sqrt(nBin_inRw)];

m_pvCorr_outlight_Rw = [mean(pvCorr_outRw_preXstm), mean(pvCorr_outRw_preXpost), mean(pvCorr_outRw_stmXpost)];
sem_pvCorr_outlight_Rw = [std(pvCorr_outRw_preXstm,0,2)/sqrt(nBin_outRw), std(pvCorr_outRw_preXpost,0,2)/sqrt(nBin_outRw), std(pvCorr_outRw_stmXpost,0,2)/sqrt(nBin_outRw)];
...
xScatterRunIn = (rand(nBin_inRun,1)-0.5)*barWidth*2.2;
xScatterRunOut = (rand(nBin_outRun,1)-0.5)*barWidth*2.2;
xScatterRwIn = (rand(nBin_inRw,1)-0.5)*barWidth*2.2;
xScatterRwOut = (rand(nBin_outRw,1)-0.5)*barWidth*2.2;
