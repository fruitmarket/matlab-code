...
% tPC_noRun = ((T.taskType == 'noRun' & isundefined(T.compIdxControl)) | (T.taskType == 'noRw' & isundefined(T.compIdxControl)) | (T.sessionOrder == '1' & ~isundefined(T.compIdxControl))) & T.idxNeurontype == 'PN' & cellfun(@(x) max(max(x)), T.pethconvSpatial)>cri_peakFR; % 1st session
% tPC_noRun = ((T.taskType == 'noRw' & isundefined(T.compIdxControl)) | T.taskType == 'noRun') & T.idxNeurontype == 'PN' & cellfun(@(x) max(max(x)), T.pethconvSpatial)>cri_peakFR; % noRun all + noRw some
tPC_noRun = ((T.taskType == 'noRun' & isundefined(T.compIdxControl)) | T.taskType == 'noRw') & T.idxNeurontype == 'PN' & cellfun(@(x) max(max(x)), T.pethconvSpatial)>cri_peakFR; % noRw all + noRun some
...
ntPC_noRw = sum(double(tPC_noRun));

noRw_PRE = cell2mat(T.rateMap1D_PRE(tPC_noRun));
noRw_STM = cell2mat(T.rateMap1D_STM(tPC_noRun));
noRw_POST = cell2mat(T.rateMap1D_POST(tPC_noRun));
...
line([0 12],[0.5 0.5],'lineStyle',':','color',colorRed);
hold on;
line([0 12],[0.7 0.7],'lineStyle',':','color',colorRed);
...
text(6, 1.15, ['Control sessions (n = ', num2str(ntPC_noRun), ')'],'fontSize',fontM);
...
text(6, 1.15, ['Control sessions (n = ', num2str(ntPC_noRw), ')'],'fontSize',fontM);
...
print('-painters','-r300','-dtiff',['f_Neuron_SpatialCorr_',datestr(now,formatOut),'_',num2str(freq),'_half_UniCtrl_fr_',num2str(cri_peakFR),'RwAll','.tif']);
