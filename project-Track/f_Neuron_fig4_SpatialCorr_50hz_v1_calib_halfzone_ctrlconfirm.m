...
% tPC_noRun = ((T.taskType == 'noRun' & isundefined(T.compIdxControl)) | T.taskType == 'noRw') & T.idxNeurontype == 'PN' & cellfun(@(x) max(max(x)), T.pethconvSpatial)>cri_meanFR; % noRw all + noRun some
...
% print('-painters','-r300','-dtiff',['f_Neuron_SpatialCorr_',datestr(now,formatOut),'_',num2str(freq),'.tif']);
print('-painters','-r300','-depsc',['f_Neuron_SpatialCorr_',datestr(now,formatOut),'_',num2str(freq),'.ai']);
close;