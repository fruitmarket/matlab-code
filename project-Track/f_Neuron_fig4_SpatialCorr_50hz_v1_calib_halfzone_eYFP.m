...
tPC_noRun = T.taskType == 'DRun' & T.idxNeurontype == 'PN' & cellfun(@(x) max(max(x)), T.pethconvSpatial)>cri_meanFR; % nanmean firing rate > 1hz
...
tPC_noRw = T.taskType == 'DRw' & T.idxNeurontype == 'PN' & cellfun(@(x) max(max(x)), T.pethconvSpatial)>cri_meanFR; % nanmean firing rate > 1hz
...
print('-painters','-r300','-dtiff',['f_Neuron_SpatialCorr_',datestr(now,formatOut),'_',num2str(freq),'_half_eYFP_fr_',num2str(cri_meanFR),'.tif']);
