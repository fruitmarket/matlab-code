...
% load('neuronList_ori_control_180204.mat');
lightLoc_Run = [37:98];
...
rateMap1D_PRE = cell2mat(T.rateMap1D_PRE);
rateMap1D_STM = cell2mat(T.rateMap1D_STM);
rateMap1D_POST = cell2mat(T.rateMap1D_POST);

inRun_rateMap1D_PRE = rateMap1D_PRE(:,lightLoc_Run);
inRun_rateMap1D_STM = rateMap1D_STM(:,lightLoc_Run);
inRun_rateMap1D_POST = rateMap1D_POST(:,lightLoc_Run);
outRun_rateMap1D_PRE = rateMap1D_PRE(:,lightLoc_RunCtrl);
outRun_rateMap1D_STM = rateMap1D_STM(:,lightLoc_RunCtrl);
outRun_rateMap1D_POST = rateMap1D_POST(:,lightLoc_RunCtrl);
inRw_rateMap1D_PRE = rateMap1D_PRE(:,lightLoc_Rw);
inRw_rateMap1D_STM = rateMap1D_STM(:,lightLoc_Rw);
inRw_rateMap1D_POST = rateMap1D_POST(:,lightLoc_Rw);
outRw_rateMap1D_PRE = rateMap1D_PRE(:,lightLoc_RwCtrl);
outRw_rateMap1D_STM = rateMap1D_STM(:,lightLoc_RwCtrl);
outRw_rateMap1D_POST = rateMap1D_POST(:,lightLoc_RwCtrl);

tPC_inRun = T.taskType == 'DRun' & T.idxNeurontype == 'PN' & (max(inRun_rateMap1D_PRE,[],2) >= cri_meanFR | max(inRun_rateMap1D_STM,[],2) >= cri_meanFR | max(inRun_rateMap1D_POST,[],2) >= cri_meanFR); % mean firing rate > 1hz
tPC_outRun = T.taskType == 'DRun' & T.idxNeurontype == 'PN' & (max(outRun_rateMap1D_PRE,[],2) >= cri_meanFR | max(outRun_rateMap1D_STM,[],2) >= cri_meanFR | max(outRun_rateMap1D_POST,[],2) >= cri_meanFR); % mean firing rate > 1hz
...
tPC_inRw = T.taskType == 'DRw' & T.idxNeurontype == 'PN' & (max(inRw_rateMap1D_PRE,[],2) >= cri_meanFR | max(inRw_rateMap1D_STM,[],2) >= cri_meanFR | max(inRw_rateMap1D_POST,[],2) >= cri_meanFR); % mean firing rate > 1hz
tPC_outRw = T.taskType == 'DRw' & T.idxNeurontype == 'PN' & (max(outRw_rateMap1D_PRE,[],2) >= cri_meanFR | max(outRw_rateMap1D_STM,[],2) >= cri_meanFR | max(outRw_rateMap1D_POST,[],2) >= cri_meanFR); % mean firing rate > 1hz
...
tPC_innoRun = T.taskType == 'noRun' & T.idxNeurontype == 'PN' & (max(inRun_rateMap1D_PRE,[],2) >= cri_meanFR | max(inRun_rateMap1D_STM,[],2) >= cri_meanFR | max(inRun_rateMap1D_POST,[],2) >= cri_meanFR); % mean firing rate > 1hz
tPC_outnoRun = T.taskType == 'noRun' & T.idxNeurontype == 'PN' & (max(outRun_rateMap1D_PRE,[],2) >= cri_meanFR | max(outRun_rateMap1D_STM,[],2) >= cri_meanFR | max(outRun_rateMap1D_POST,[],2) >= cri_meanFR); % mean firing rate > 1hz
...
tPC_innoRw = T.taskType == 'noRw' & T.idxNeurontype == 'PN' & (max(inRw_rateMap1D_PRE,[],2) >= cri_meanFR | max(inRw_rateMap1D_STM,[],2) >= cri_meanFR | max(inRw_rateMap1D_POST,[],2) >= cri_meanFR); % mean firing rate > 1hz
tPC_outnoRw = T.taskType == 'noRw' & T.idxNeurontype == 'PN' & (max(outRw_rateMap1D_PRE,[],2) >= cri_meanFR | max(outRw_rateMap1D_STM,[],2) >= cri_meanFR | max(outRw_rateMap1D_POST,[],2) >= cri_meanFR); % mean firing rate > 1hz
...
% print('-painters','-r300','-dtiff',['f_Neuron_SpatialCorr_',datestr(now,formatOut),'_',num2str(freq),'_half_threshold_fr_',num2str(cri_meanFR),'.tif']);
