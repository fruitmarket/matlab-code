...
tPC_outnoRun = T.taskType == 'noRun' & T.idxNeurontype == 'PN' & cellfun(@(x) sum(x(1:90)), T.outCtrlSpike) > cri_spike; % mean firing rate > 1hz
...
print('-painters','-r300','-dtiff',['f_Neuron_SpatialCorr_',datestr(now,formatOut),'_50Hz_block_bin5sd2_calib_threshold100','.tif']);
