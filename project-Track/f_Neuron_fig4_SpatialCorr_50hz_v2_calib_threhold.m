...
% group_Rw = [ones(ntPC_DRw,1);2*ones(ntPC_DRw,1);3*ones(ntPC_innoRw,1)];
...
[p_totalRun(3), table, stats] = kruskalwallis([rCorr_innoRun_preXstm(~isnan(rCorr_innoRun_preXstm)); rCorr_innoRun_preXpost(~isnan(rCorr_innoRun_preXpost)); rCorr_innoRun_stmXpost(~isnan(rCorr_innoRun_stmXpost))],[ones(sum(double(~isnan(rCorr_innoRun_preXstm))),1); 2*ones(sum(double(~isnan(rCorr_innoRun_preXpost))),1); 3*ones(sum(double(~isnan(rCorr_innoRun_stmXpost))),1)],'off');
% result_Run = multcompare(stats,'ctype','hsd','Display','off');
% result_Run = multcompare(stats,'ctype','bonferroni','Display','off');
result_Run = multcompare(stats,'ctype','lsd','Display','off');
...
[p_totalRw(3), table, stats] = kruskalwallis([rCorr_innoRw_preXstm(~isnan(rCorr_innoRw_preXstm)); rCorr_innoRw_preXpost(~isnan(rCorr_innoRw_preXpost)); rCorr_innoRw_stmXpost(~isnan(rCorr_innoRw_stmXpost))],[ones(sum(double(~isnan(rCorr_innoRw_preXstm))),1); 2*ones(sum(double(~isnan(rCorr_innoRw_preXpost))),1); 3*ones(sum(double(~isnan(rCorr_innoRw_stmXpost))),1)],'off');
% result_Rw = multcompare(stats,'ctype','hsd','Display','off');
% result_Rw = multcompare(stats,'ctype','bonferroni','Display','off');
result_Rw = multcompare(stats,'ctype','lsd','Display','off');
...
xScatterRunIn = (rand(ntPC_inRun,1)-0.5)*barWidth*0.5;
xScatterRunOut = (rand(ntPC_outRun,1)-0.5)*barWidth*0.5;
xScatterRwIn = (rand(ntPC_inRw,1)-0.5)*barWidth*0.5;
xScatterRwOut = (rand(ntPC_outRw,1)-0.5)*barWidth*0.5;
xScatterNoRun = (rand(ntPC_innoRun,1)-0.5)*barWidth*0.5;
xScatterNoRw = (rand(ntPC_innoRw,1)-0.5)*barWidth*0.5;
...
plot(9+xScatterNoRun, rCorr_innoRun_preXstm, '.','markerSize',dotS,'markerFaceColor',colorWhite,'markerEdgeColor',colorGray);
hold on;
plot(10+xScatterNoRun, rCorr_innoRun_preXpost, '.','markerSize',dotS,'markerFaceColor',colorWhite,'markerEdgeColor',colorGray);
hold on;
...
plot(9+xScatterNoRw, rCorr_innoRw_preXstm, '.','markerSize',dotS,'markerFaceColor',colorWhite,'markerEdgeColor',colorGray);
hold on;
plot(10+xScatterNoRw, rCorr_innoRw_preXpost, '.','markerSize',dotS,'markerFaceColor',colorWhite,'markerEdgeColor',colorGray);
hold on;
...
print('-painters','-r300','-dtiff',['f_Neuron_SpatialCorr_',datestr(now,formatOut),'_50Hz_zone_bin5sd2_calib_threshold','.tif']);
