...
inNoZone_PRE_Run = noRun_PRE(:,lightLoc_Run(1):lightLoc_Run(2));
inNoZone_STM_Run = noRun_STM(:,lightLoc_Run(1):lightLoc_Run(2));
inNoZone_POST_Run = noRun_POST(:,lightLoc_Run(1):lightLoc_Run(2));

outNoZone_PRE_Run = noRun_PRE(:,[1:lightLoc_Run(1)-1,lightLoc_Run(2)+1:nBin_50hz]);
outNoZone_STM_Run = noRun_STM(:,[1:lightLoc_Run(1)-1,lightLoc_Run(2)+1:nBin_50hz]);
outNoZone_POST_Run = noRun_POST(:,[1:lightLoc_Run(1)-1,lightLoc_Run(2)+1:nBin_50hz]);

[rCorr_inNoRun_preXstm,rCorr_inNoRun_preXpost,rCorr_inNoRun_stmXpost] = deal([]);
[rCorr_outNoRun_preXstm,rCorr_outNoRun_preXpost,rCorr_outNoRun_stmXpost] = deal([]);

nBin_inNoRun = size(inNoZone_PRE_Run,2);
nBin_outNoRun = size(outNoZone_PRE_Run,2);

for iCell = 1:ntPC_noRun
    rCorr_inNoRun_preXstm(iCell,1) = corr(inNoZone_PRE_Run(iCell,:)',inNoZone_STM_Run(iCell,:)','rows','complete','type','Pearson');
    rCorr_inNoRun_preXpost(iCell,1) = corr(inNoZone_PRE_Run(iCell,:)',inNoZone_POST_Run(iCell,:)','rows','complete','type','Pearson');
    rCorr_inNoRun_stmXpost(iCell,1) = corr(inNoZone_STM_Run(iCell,:)',inNoZone_POST_Run(iCell,:)','rows','complete','type','Pearson');
end

for iCell = 1:ntPC_noRun
    rCorr_outNoRun_preXstm(iCell,1) = corr(outNoZone_PRE_Run(iCell,:)',outNoZone_STM_Run(iCell,:)','rows','complete','type','Pearson');
    rCorr_outNoRun_preXpost(iCell,1) = corr(outNoZone_PRE_Run(iCell,:)',outNoZone_POST_Run(iCell,:)','rows','complete','type','Pearson');
    rCorr_outNoRun_stmXpost(iCell,1) = corr(outNoZone_STM_Run(iCell,:)',outNoZone_POST_Run(iCell,:)','rows','complete','type','Pearson');
end

%%%
% rCorr_inNoRun_preXstm = rCorr_inNoRun_preXstm.^2;
% rCorr_inNoRun_preXpost = rCorr_inNoRun_preXpost.^2;
% rCorr_inNoRun_stmXpost = rCorr_inNoRun_stmXpost.^2;

% rCorr_outNoRun_preXstm = rCorr_outNoRun_preXstm.^2;
% rCorr_outNoRun_preXpost = rCorr_outNoRun_preXpost.^2;
% rCorr_outNoRun_stmXpost = rCorr_outNoRun_stmXpost.^2;
...
inNoZone_PRE_Rw = noRw_PRE(:,lightLoc_Rw(1):lightLoc_Rw(2));
inNoZone_STM_Rw = noRw_STM(:,lightLoc_Rw(1):lightLoc_Rw(2));
inNoZone_POST_Rw = noRw_POST(:,lightLoc_Rw(1):lightLoc_Rw(2));

outNoZone_PRE_Rw = noRw_PRE(:,[1:lightLoc_Rw(1)-1,lightLoc_Rw(2)+1:nBin_50hz]);
outNoZone_STM_Rw = noRw_STM(:,[1:lightLoc_Rw(1)-1,lightLoc_Rw(2)+1:nBin_50hz]);
outNoZone_POST_Rw = noRw_POST(:,[1:lightLoc_Rw(1)-1,lightLoc_Rw(2)+1:nBin_50hz]);

[rCorr_inNoRw_preXstm,rCorr_inNoRw_preXpost,rCorr_inNoRw_stmXpost] = deal([]);
[rCorr_outNoRw_preXstm,rCorr_outNoRw_preXpost,rCorr_outNoRw_stmXpost] = deal([]);

nBin_inNoRw = size(inNoZone_PRE_Rw,2);
nBin_outNoRw = size(outNoZone_PRE_Rw,2);

for iCell = 1:ntPC_noRw
    rCorr_inNoRw_preXstm(iCell,1) = corr(inNoZone_PRE_Rw(iCell,:)',inNoZone_STM_Rw(iCell,:)','rows','complete','type','Pearson');
    rCorr_inNoRw_preXpost(iCell,1) = corr(inNoZone_PRE_Rw(iCell,:)',inNoZone_POST_Rw(iCell,:)','rows','complete','type','Pearson');
    rCorr_inNoRw_stmXpost(iCell,1) = corr(inNoZone_STM_Rw(iCell,:)',inNoZone_POST_Rw(iCell,:)','rows','complete','type','Pearson');
end

for iCell = 1:ntPC_noRw
    rCorr_outNoRw_preXstm(iCell,1) = corr(outNoZone_PRE_Rw(iCell,:)',outNoZone_STM_Rw(iCell,:)','rows','complete','type','Pearson');
    rCorr_outNoRw_preXpost(iCell,1) = corr(outNoZone_PRE_Rw(iCell,:)',outNoZone_POST_Rw(iCell,:)','rows','complete','type','Pearson');
    rCorr_outNoRw_stmXpost(iCell,1) = corr(outNoZone_STM_Rw(iCell,:)',outNoZone_POST_Rw(iCell,:)','rows','complete','type','Pearson');
end
...
m_rCorr_inNoRun = [nanmean(rCorr_inNoRun_preXstm), nanmean(rCorr_inNoRun_preXpost), nanmean(rCorr_inNoRun_stmXpost)];
sem_rCorr_inNoRun = [nanstd(rCorr_inNoRun_preXstm,0,1)/sqrt(sum(double(~isnan(rCorr_inNoRun_preXstm)))), nanstd(rCorr_inNoRun_preXpost,0,1)/sqrt(sum(double(~isnan(rCorr_inNoRun_preXpost)))), nanstd(rCorr_inNoRun_stmXpost,0,1)/sqrt(sum(double(~isnan(rCorr_inNoRun_stmXpost))))];

m_rCorr_outNoRun = [nanmean(rCorr_outNoRun_preXstm), nanmean(rCorr_outNoRun_preXpost), nanmean(rCorr_outNoRun_stmXpost)];
sem_rCorr_outNoRun = [nanstd(rCorr_outNoRun_preXstm,0,1)/sqrt(sum(double(~isnan(rCorr_outNoRun_preXstm)))), nanstd(rCorr_outNoRun_preXpost,0,1)/sqrt(sum(double(~isnan(rCorr_outNoRun_preXpost)))), nanstd(rCorr_outNoRun_stmXpost,0,1)/sqrt(sum(double(~isnan(rCorr_outNoRun_stmXpost))))];

m_rCorr_inNoRw = [nanmean(rCorr_inNoRw_preXstm), nanmean(rCorr_inNoRw_preXpost), nanmean(rCorr_inNoRw_stmXpost)];
sem_rCorr_inNoRw = [nanstd(rCorr_inNoRw_preXstm,0,1)/sqrt(sum(double(~isnan(rCorr_inNoRw_preXstm)))), nanstd(rCorr_inNoRw_preXpost,0,1)/sqrt(sum(double(~isnan(rCorr_inNoRw_preXpost)))), nanstd(rCorr_inNoRw_stmXpost,0,1)/sqrt(sum(double(~isnan(rCorr_inNoRw_stmXpost))))];

m_rCorr_outNoRw = [nanmean(rCorr_outNoRw_preXstm), nanmean(rCorr_outNoRw_preXpost), nanmean(rCorr_outNoRw_stmXpost)];
sem_rCorr_outNoRw = [nanstd(rCorr_outNoRw_preXstm,0,1)/sqrt(sum(double(~isnan(rCorr_outNoRw_preXstm)))), nanstd(rCorr_outNoRw_preXpost,0,1)/sqrt(sum(double(~isnan(rCorr_outNoRw_preXpost)))), nanstd(rCorr_outRw_stmXpost,0,1)/sqrt(sum(double(~isnan(rCorr_outRw_stmXpost))))];
...
% 
% print('-painters','-r300','-dtiff',['f_CellReports_SpatialCorr_50Hz_v2_hsd_',datestr(now,formatOut),'.tif']);
