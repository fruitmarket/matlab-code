...
tPC_DRun = T.taskType == 'DRun' & T.idxNeurontype == 'PN' & cellfun(@(x) max(max(x)), T.pethconvSpatial)>cri_meanFR; % nanmean firing rate > 1hz
...
nBin_inlightRun = size(inZone_PRE_Run,2);
nBin_outlightRun = size(outZone_PRE_Run,2);

for iCell = 1:ntPC_DRun
    rCorr_inRun_preXstm(iCell,1) = corr(inZone_PRE_Run(iCell,:)',inZone_STM_Run(iCell,:)','rows','complete','type','Pearson');
    rCorr_inRun_preXpost(iCell,1) = corr(inZone_PRE_Run(iCell,:)',inZone_POST_Run(iCell,:)','rows','complete','type','Pearson');
    rCorr_inRun_stmXpost(iCell,1) = corr(inZone_STM_Run(iCell,:)',inZone_POST_Run(iCell,:)','rows','complete','type','Pearson');
end

for iCell = 1:ntPC_DRun
    rCorr_outRun_preXstm(iCell,1) = corr(outZone_PRE_Run(iCell,:)',outZone_STM_Run(iCell,:)','rows','complete','type','Pearson');
    rCorr_outRun_preXpost(iCell,1) = corr(outZone_PRE_Run(iCell,:)',outZone_POST_Run(iCell,:)','rows','complete','type','Pearson');
    rCorr_outRun_stmXpost(iCell,1) = corr(outZone_STM_Run(iCell,:)',outZone_POST_Run(iCell,:)','rows','complete','type','Pearson');
end
...
tPC_DRw = T.taskType == 'DRw' & T.idxNeurontype == 'PN' & cellfun(@(x) max(max(x)), T.pethconvSpatial)>cri_meanFR; % nanmean firing rate > 1hz
...
nBin_inlightRw = size(inZone_PRE_Rw,2);
nBin_outlightRw = size(outZone_PRE_Rw,2);

for iCell = 1:ntPC_DRw
    rCorr_inRw_preXstm(iCell,1) = corr(inZone_PRE_Rw(iCell,:)',inZone_STM_Rw(iCell,:)','rows','complete','type','Pearson');
    rCorr_inRw_preXpost(iCell,1) = corr(inZone_PRE_Rw(iCell,:)',inZone_POST_Rw(iCell,:)','rows','complete','type','Pearson');
    rCorr_inRw_stmXpost(iCell,1) = corr(inZone_STM_Rw(iCell,:)',inZone_POST_Rw(iCell,:)','rows','complete','type','Pearson');
end

for iCell = 1:ntPC_DRw
    rCorr_outRw_preXstm(iCell,1) = corr(outZone_PRE_Rw(iCell,:)',outZone_STM_Rw(iCell,:)','rows','complete','type','Pearson');
    rCorr_outRw_preXpost(iCell,1) = corr(outZone_PRE_Rw(iCell,:)',outZone_POST_Rw(iCell,:)','rows','complete','type','Pearson');
    rCorr_outRw_stmXpost(iCell,1) = corr(outZone_STM_Rw(iCell,:)',outZone_POST_Rw(iCell,:)','rows','complete','type','Pearson');
end
...
print('-painters','-r300','-dtiff',['f_Neuron_SpatialCorr_',datestr(now,formatOut),'_8Hz_block','.tif']);
