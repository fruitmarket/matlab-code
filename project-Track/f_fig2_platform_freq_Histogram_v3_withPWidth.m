...
nCol = 2;
nRow = 6;
hPlotPN(1) = axes('Position',axpt(nCol,nRow,1,1,[0.10 0.10 0.85 0.8],midInterval));
...
hPlotPN(2) = axes('Position',axpt(2,1,1,1,axpt(nCol,nRow,2,1,[0.10 0.10 0.85 0.8],midInterval),midInterval));
...
hPlotPN(3) = axes('Position',axpt(2,1,2,1,axpt(nCol,nRow,2,1,[0.10 0.10 0.85 0.8],midInterval),midInterval));
...
hPlotPN(4) = axes('Position',axpt(nCol,nRow,1,2,[0.10 0.10 0.85 0.8],midInterval));
...
text(15, yLim(4)*0.8,['n = ',num2str(ninactPN)],'fontSize',fontL);
xlabel('Time (ms)','fontSize',fontL);
ylabel('Spikes/bin','fontSize',fontL);
title('PN_inactivated','interpreter','none','fontSize',fontL,'fontWeight','bold');

hPlotPN(5) = axes('Position',axpt(nCol,nRow,1,3,[0.10 0.10 0.85 0.8],midInterval));
...
text(15, yLim(5)*0.8,['n = ',num2str(nnoPN)],'fontSize',fontL);
xlabel('Time (ms)','fontSize',fontL);
ylabel('Spikes/bin','fontSize',fontL);
...
hPlotPN(6) = axes('Position',axpt(nCol,nRow,2,2,[0.10 0.10 0.85 0.8],midInterval));
...
hold on;
hBarPN(6) = bar(xpt,m_actPN_peth10ms,'histc');
...
xlabel('Time (ms)','fontSize',fontL);
ylabel('Spikes/bin','fontSize',fontL);
title('10 ms @ 2 Hz','fontSize',fontL,'interpreter','none','fontWeight','bold');

hPlotPN(7) = axes('Position',axpt(nCol,nRow,2,3,[0.10 0.10 0.85 0.8],midInterval));
...
hold on;
hBarPN(7) = bar(xpt,m_actPN_peth50ms,'histc');
errorbarJun(xpt+1,m_actPN_peth50ms,sem_actPN_peth50ms,1,0.4,colorDarkGray);
xlabel('Time (ms)','fontSize',fontL);
ylabel('Spikes/bin','fontSize',fontL);
title('50 ms @ 2 Hz','fontSize',fontL,'interpreter','none','fontWeight','bold');
%%
...
set(hPlotPN(2),'XLim',[-10, 55],'XTick',[-10, 0, 10, 20, 55],'YLim',[0 yLim(2)],'fontSize',fontL);
set(hPlotPN(3),'XLim',[-10, 55],'XTick',[-10, 0, 10, 20, 55],'YLim',[0 yLim(3)],'fontSize',fontL);
set(hPlotPN(4),'YLim',[0 yLim(4)],'fontSize',fontL);
set(hPlotPN(5),'YLim',[0 yLim(5)],'fontSize',fontL);

print('-painters','-r300','-dtiff',['final_fig2_platform_8hz_pulseWidth',datestr(now,formatOut),'.tif']);
