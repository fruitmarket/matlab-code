...
% DRunPN = T.taskType == 'DRun' & T.idxNeurontype == 'PN' & T.idxPeakFR & T.idxPlaceField & T.idxTotalSpikeNum; % place cell
DRunPN = T.taskType == 'DRun' & T.idxNeurontype == 'PN'; % all recorded pyramidal neuron
...
set(hPlot(3),'YLim',[0,yLim(3)],'YTick',[0:4]);
set(hPlot(4),'YLim',[0,yLim(4)],'YTick',[0:4]);

print('-painters','-r300','-dtiff',['final_figXX_InOutZone_DRun_v2_norm_',datestr(now,formatOut),'.tif']);
print('-painters','-r300','-depsc',['final_figXX_InOutZone_DRun_v2_norm_',datestr(now,formatOut),'.ai']);
close;