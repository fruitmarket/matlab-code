...
load neuronList_ori50hz_170928.mat
formatOut = 'yymmdd';

% cellID = [45 66 65 125 79 26]; % DRun
cellID = [30 24 19 36 93 118]; % DRw
nCell = length(cellID);
...
wideInterval = wideInterval + 0.09;
for iCell = 1:nCell
...
%         uistack(rec,'bottom');
    ...
    set(hPlot(1),'Box','off','TickDir','out','fontSize',fontS,'XLim',[-100 1000],'XTick',[0,500,1000],'YLim',[0,90],'YTick',[0:30:90]);
...
%     set(hPlot(4),'XLim',[0,124],'XTick',[0:20:120],'YLim',[0, ylimpethSpatial]);
    ...
    cd('D:\Dropbox\SNL\P2_Track\example_track_PF_DRw');
    print('-painters','-r300','-dtiff',[datestr(now,formatOut),'_placeCell_50hz_Example_',num2str(iCell),'.tif']);
    print('-painters','-r300','-depsc',[datestr(now,formatOut),'_placeCell_50hz_Example_',num2str(iCell),'.ai']);
    close;
end