...
text(4, 1.05,['n (light resp.) = ',num2str(nrespPC_DRun)],'fontSize',fontS,'color',colorDarkBlue);
text(4, 1.00,['n (noRun) = ',num2str(ntPC_noRun)],'fontSize',fontS,'color',colorBlack);

% hPVCorr(2) = axes('Position',axpt(nCol,nRow,2,1,[0.1 0.1 0.855 0.85],wideInterval));
% bar([1,3,5],m_respPC_DRun_PVcorr,barWidth,'faceColor',colorLightBlue);
% hold on;
% title('Light resp. (DRun)','fontSize',fontS,'fontWeight','bold');
% % ylabel('PV correlation','fontSize',fontS);
% text(3, 1.1,['n (DRun) = ',num2str(nrespPC_DRun)],'fontSize',fontS,'color',colorDarkBlue);
...
text(4, 1.05,['n (light resp.) = ',num2str(nrespPC_DRw)],'fontSize',fontS,'color',colorDarkBlue);
text(4, 1.0,['n (noRw) = ',num2str(ntPC_noRw)],'fontSize',fontS,'color',colorBlack);

% hPVCorr(4) = axes('Position',axpt(nCol,nRow,4,1,[0.1 0.1 0.855 0.85],wideInterval));
% bar([1,3,5],m_respPC_DRw_PVcorr,barWidth,'faceColor',colorLightBlue);
% hold on;
% title('Light resp. (DRw)','fontSize',fontS,'fontWeight','bold');
% % ylabel('PV correlation','fontSize',fontS);
% text(3, 1.1,['n (DRw) = ',num2str(nrespPC_DRw)],'fontSize',fontS,'color',colorDarkBlue);

set(hPVCorr(1),'TickDir','out','Box','off','XLim',[0,14],'YLim',[0 1.15],'XTick',[2, 7, 12],'XTickLabel',[{'PRExSTM','PRExPOST','STMxPOST'}],'fontSize',fontS);
set(hPVCorr(2),'TickDir','out','Box','off','XLim',[0,14],'YLim',[0 1.15],'XTick',[2, 7, 12],'XTickLabel',[{'PRExSTM','PRExPOST','STMxPOST'}],'fontSize',fontS);
% set(hPVCorr(2),'TickDir','out','Box','off','XLim',[0,6],'YLim',[0 1.15],'XTick',[1, 3, 5],'XTickLabel',[{'preXstm','preXpost','stmXpost'}],'YTickLabel',[],'fontSize',fontS);
% set(hPVCorr(4),'TickDir','out','Box','off','XLim',[0,6],'YLim',[0 1.15],'XTick',[1, 3, 5],'XTickLabel',[{'preXstm','preXpost','stmXpost'}],'YTickLabel',[],'fontSize',fontS);

% print('-painters','-r300','-dtiff',['final_fig5_placeField_PVCorr_DRunDRw_',datestr(now,formatOut),'.tif']);
% print('-painters','-r300','-depsc',['final_fig5_placeField_PVCorr_DRunDRw_',datestr(now,formatOut),'.ai']);
% close;