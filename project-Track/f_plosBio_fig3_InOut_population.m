...
nInzone_act_Run = size(fr_Inzone_act_Run,1); % after threshold applied
nInzone_ina_Run = size(fr_Inzone_ina_Run,1);
nInzone_no_Run = size(fr_Inzone_no_Run,1);

Run_a = sum(double(DRunPN & idx_inc & idx_pPRExSTM)); % population change (before thresould)
...
nInzone_act_Rw = length(fr_Inzone_act_Rw); % after threshold applied
nInzone_ina_Rw = length(fr_Inzone_ina_Rw);
nInzone_no_Rw = length(fr_Inzone_no_Rw);

Rw_a = sum(double(DRwPN & idx_inc & idx_pPRExSTM)); % before threshold applied
