...
% cellID = [101]; % DRun
cellID = [101 324 345 506 630 321 365 504 617 239 477 582 658]; % DRun
% cellID = [335 473 498 639 833 842 17 24 481 483 497 749 336 496 803]; % DRw
...
if(regexp(cellDir,'Run'))
    cd('E:\Dropbox\SNL\P2_Track\example_track8hz_Run_fig');
    ...
    cd('E:\Dropbox\SNL\P2_Track\example_track8hz_Rw_fig');
    print('-painters','-r300','-dtiff',[datestr(now,formatOut),'_placeCell_50hz_Example_',num2str(iCell),'.tif']);
    print('-painters','-r300','-depsc',[datestr(now,formatOut),'_placeCell_50hz_Example_',num2str(iCell),'.ai']);
    close;
end
