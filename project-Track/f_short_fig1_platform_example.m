...
%     hold on;
    hLpatch(1) = patch([125*(iLight-1), 125*(iLight-1)+10,125*(iLight-1)+10,125*(iLight-1)],[yLimSpike(2)*0.9, yLimSpike(2)*0.9, yLimSpike(2), yLimSpike(2)],colorLLightBlue,'EdgeColor','none');
    hold on;
end
...
%% all light aligned
hFreq8hz(2) = axes('Position',axpt(3,8,1:2,3:4,axpt(nCol,nRow,1,1,[0.1 0.1 0.85 0.85],midInterval),wideInterval));
hLpatch(3) = patch([0 10 10 0],[0 0 nLight nLight],colorLLightBlue,'EdgeColor','none');
hold on;
...
print('-painters','-r300','-dtiff',['final_fig1_example_',datestr(now,formatOut),'_v2.tif']);
print('-painters','-r300','-depsc',['final_fig1_example_',datestr(now,formatOut),'_v2.ai']);
close;
