...
%%
fHandle = figure('PaperUnits','centimeters','Paperposition',[0 0 4 5]);
hPlot(1) = axes('Position',axpt(3,2,1:2,1,[0.3 0.25 0.85 0.7],tightInterval));
bar(xBar,m_frInZone_Run,'BarWidth',0.6,'LineStyle','-','edgeColor',colorBlack,'FaceColor',colorDarkGray,'lineWidth',lineWidth);
hold on;
for iBlock = 1:3
...
text(5, yLim(1)*0.95,['n = ',num2str(nInzone_Run)],'fontSize',fontM);
ylabel('FR (Hz)','fontSize',fontM);
...
%%
hPlot(2) = axes('Position',axpt(3,2,1:2,2,[0.3 0.15 0.85 0.7],tightInterval));
bar(xBar,m_frInZone_Rw,'BarWidth',0.6,'LineStyle','-','edgeColor',colorBlack,'FaceColor',colorDarkGray,'lineWidth',lineWidth);
hold on;
for iBlock = 1:3
...
text(5, yLim(2)*0.95,['n = ',num2str(nInzone_Rw)],'fontSize',fontM);
ylabel('FR (Hz)','fontSize',fontM);
% title('In-zone (DRwPN)','fontSize',fontM,'fontWeight','bold');

set(hPlot(1),'YLim',[0,yLim(1)],'YTick',[0,yLim(1)]);
set(hPlot(2),'YLim',[0,yLim(2)],'YTick',[0,yLim(1)]);
set(hPlot,'TickLength',[0.03 0.03]);
set(hPlot,'Box','off','TickDir','out','XLim',[0,6],'XTick',[1,3,5],'XTickLabel',{'PRE','STIM','POST'},'fontSize',fontM);


print('-painters','-r300','-dtiff',['f_Neuron_fig2_InOutZoneInset_50hz_',datestr(now,formatOut),'.tif']);
print('-painters','-r300','-depsc',['f_Neuron_fig2_InOutZoneInset_50hz_',datestr(now,formatOut),'.ai']);
close;