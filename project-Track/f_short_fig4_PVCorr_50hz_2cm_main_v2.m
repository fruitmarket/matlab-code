...
sem_pvCorr_inlight_Run = [nanstd(pvCorr_inRun_preXstm,0,2)/sqrt(nBin_inlightRun), nanstd(pvCorr_inRun_preXpost)/sqrt(nBin_inlightRun), nanstd(pvCorr_inRun_stmXpost)/sqrt(nBin_inlightRun)];

m_pvCorr_outlight_Run = [nanmean(pvCorr_outRun_preXstm), nanmean(pvCorr_outRun_preXpost), nanmean(pvCorr_outRun_stmXpost)];
sem_pvCorr_outlight_Run = [nanstd(pvCorr_outRun_preXstm,0,2)/sqrt(nBin_outlightRun), nanstd(pvCorr_outRun_preXpost)/sqrt(nBin_outlightRun), nanstd(pvCorr_outRun_stmXpost)/sqrt(nBin_outlightRun)];
...
sem_pvCorr_noRw = [nanstd(pvCorr_noRw_preXstm,0,2)/sqrt(nBin_ctrl), nanstd(pvCorr_noRw_preXpost,0,2)/sqrt(nBin_ctrl), nanstd(pvCorr_noRw_stmXpost,0,2)/sqrt(nBin_ctrl)];
...
print('-painters','-r300','-dtiff',['f_short_suppleXX_PVCorr_InOutTotalZone_',datestr(now,formatOut),'_2cm2HzNormKW.tif']);
print('-painters','-r300','-depsc',['f_short_suppleXX_PVCorr_InOutTotalZone_',datestr(now,formatOut),'_2cm2HzNormKW.ai']);
close;