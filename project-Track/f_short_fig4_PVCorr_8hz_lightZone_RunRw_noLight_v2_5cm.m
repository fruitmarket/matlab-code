...
load('neuronList_ori_171205_5cm.mat');
load('D:\Dropbox\SNL\P2_Track\myParameters.mat');
formatOut = 'yymmdd';
lightLoc_Run = [floor(20*pi*5/6) ceil(20*pi*8/6)]/4;
lightLoc_Rw = [floor(20*pi*9/6) ceil(20*pi*10/6)]/4;
lightLoc_Rw = [floor(lightLoc_Rw(1)), ceil(lightLoc_Rw(2))];
...
outlightZone_PRE_Run = tPC_DRun_PRE(:,[1:lightLoc_Run(1)-1,lightLoc_Run(2)+1:31]);
outlightZone_STM_Run = tPC_DRun_STM(:,[1:lightLoc_Run(1)-1,lightLoc_Run(2)+1:31]);
outlightZone_POST_Run = tPC_DRun_POST(:,[1:lightLoc_Run(1)-1,lightLoc_Run(2)+1:31]);
...
for iCol = 1:diff(lightLoc_Run)+1
...
for iCol = 1:(31-diff(lightLoc_Run)-1)
...
sem_pvCorr_inlight_Run = [nanstd(pvCorr_inRun_preXstm,0,2)/sqrt(9), nanstd(pvCorr_inRun_preXpost)/sqrt(9), nanstd(pvCorr_inRun_stmXpost)/sqrt(9)];

m_pvCorr_outlight_Run = [nanmean(pvCorr_outRun_preXstm), nanmean(pvCorr_outRun_preXpost), nanmean(pvCorr_outRun_stmXpost)];
sem_pvCorr_outlight_Run = [nanstd(pvCorr_outRun_preXstm,0,2)/sqrt(22), nanstd(pvCorr_outRun_preXpost)/sqrt(22), nanstd(pvCorr_outRun_stmXpost)/sqrt(22)];
...
outlightZone_PRE_Rw = tPC_DRw_PRE(:,[1:lightLoc_Rw(1)-1,lightLoc_Rw(2)+1:31]);
outlightZone_STM_Rw = tPC_DRw_STM(:,[1:lightLoc_Rw(1)-1,lightLoc_Rw(2)+1:31]);
outlightZone_POST_Rw = tPC_DRw_POST(:,[1:lightLoc_Rw(1)-1,lightLoc_Rw(2)+1:31]);
...
for iCol = 1:(31-diff(lightLoc_Rw)-1)
...
sem_pvCorr_inlight_Rw = [nanstd(pvCorr_inRw_preXstm,0,2)/sqrt(5), nanstd(pvCorr_inRw_preXpost)/sqrt(5), nanstd(pvCorr_inRw_stmXpost)/sqrt(5)];

m_pvCorr_outlight_Rw = [mean(pvCorr_outRw_preXstm), mean(pvCorr_outRw_preXpost), mean(pvCorr_outRw_stmXpost)];
sem_pvCorr_outlight_Rw = [std(pvCorr_outRw_preXstm,0,2)/sqrt(26), std(pvCorr_outRw_preXpost)/sqrt(26), std(pvCorr_outRw_stmXpost)/sqrt(26)];
...
m_pvCorr_noRun = [nanmean(pvCorr_noRun_preXstm), nanmean(pvCorr_noRun_preXpost), nanmean(pvCorr_noRun_stmXpost)];
sem_pvCorr_noRun = [nanstd(pvCorr_noRun_preXstm,0,2)/sqrt(31), nanstd(pvCorr_noRun_preXpost,0,2)/sqrt(31), nanstd(pvCorr_noRun_stmXpost,0,2)/sqrt(31)];
...
sem_pvCorr_noRw = [nanstd(pvCorr_noRw_preXstm,0,2)/sqrt(31), nanstd(pvCorr_noRw_preXpost,0,2)/sqrt(31), nanstd(pvCorr_noRw_stmXpost,0,2)/sqrt(31)];
...
group_inRun = [ones(nInRun,1);2*ones(nInRun,1);3*ones(nInRun,1)];
group_outRun = [ones(nOutRun,1);2*ones(nOutRun,1);3*ones(nOutRun,1)];
group_inRw = [ones(nInRw,1);2*ones(nInRw,1);3*ones(nInRw,1)];
group_outRw = [ones(nOutRw,1);2*ones(nOutRw,1);3*ones(nOutRw,1)];
group_total = [ones(nTotal,1);2*ones(nTotal,1);3*ones(nTotal,1)];


% Run session
% [p_Run(1), ~, stats] = anova1([pvCorr_inRun_preXstm,pvCorr_inRun_preXpost,pvCorr_inRun_stmXpost],group_inRun,'off');
[p_Run(1), ~, stats1] = kruskalwallis([pvCorr_inRun_preXstm,pvCorr_inRun_preXpost,pvCorr_inRun_stmXpost],group_inRun,'off');
result_Run = multcompare(stats1,'Display','off');
p_anova_Run(:,1) = result_Run(:,end);

% [p_Run(2), ~, stats] = anova1([pvCorr_outRun_preXstm,pvCorr_outRun_preXpost,pvCorr_outRun_stmXpost],group_outRun,'off');
[p_Run(2), ~, stats] = kruskalwallis([pvCorr_outRun_preXstm,pvCorr_outRun_preXpost,pvCorr_outRun_stmXpost],group_outRun,'off');
result_Run = multcompare(stats,'Alpha',0.05,'ctype','bonferroni','Display','off');
p_anova_Run(:,2) = result_Run(:,end);

% [p_Run(3), ~, stats] = anova1([pvCorr_noRun_preXstm,pvCorr_noRun_preXpost,pvCorr_noRun_stmXpost],group_total,'off');
[p_Run(3), ~, stats] = kruskalwallis([pvCorr_noRun_preXstm,pvCorr_noRun_preXpost,pvCorr_noRun_stmXpost],group_total,'off');
result_Run = multcompare(stats,'Alpha',0.05,'ctype','bonferroni','Display','off');
p_anova_Run(:,3) = result_Run(:,end);

% Rw session
% [p_Rw(1), ~, stats] = kruskalwallis([pvCorr_inRw_preXstm,pvCorr_inRw_preXpost,pvCorr_inRw_stmXpost],group_inRw,'off');
[p_Rw(1), ~, stats] = kruskalwallis([pvCorr_inRw_preXstm,pvCorr_inRw_preXpost,pvCorr_inRw_stmXpost],group_inRw,'off');
result_Rw = multcompare(stats,'Alpha',0.05,'ctype','bonferroni','Display','off');
p_anova_Rw(:,1) = result_Rw(:,end);

% [p_Rw(2), ~, stats] = anova1([pvCorr_outRw_preXstm,pvCorr_outRw_preXpost,pvCorr_outRw_stmXpost],group_outRw,'off');
[p_Rw(2), ~, stats] = kruskalwallis([pvCorr_outRw_preXstm,pvCorr_outRw_preXpost,pvCorr_outRw_stmXpost],group_outRw,'off');
result_Rw = multcompare(stats,'Alpha',0.05,'ctype','bonferroni','Display','off');
p_anova_Rw(:,2) = result_Rw(:,end);

% [p_Rw(3), ~, stats] = anova1([pvCorr_noRw_preXstm,pvCorr_noRw_preXpost,pvCorr_noRw_stmXpost],group_total,'off');
[p_Rw(3), ~, stats] = kruskalwallis([pvCorr_noRw_preXstm,pvCorr_noRw_preXpost,pvCorr_noRw_stmXpost],group_total,'off');
result_Rw = multcompare(stats,'Alpha',0.05,'ctype','bonferroni','Display','off');
...
hold on;
errorbarJun([6,7,8],m_pvCorr_outlight_Run,sem_pvCorr_outlight_Run,0.3,1.0,colorBlack);
hold on;
...
hold on;
errorbarJun([6,7,8],m_pvCorr_outlight_Rw,sem_pvCorr_outlight_Rw,0.3,1.0,colorBlack);
hold on;
...
text(-2, -0.3,'p12 = Nosig','fontSize',fontM,'color',colorBlack);
text(-2, -0.5,'p13 = Nosig','fontSize',fontM,'color',colorBlack);
text(-2, -0.7,'p23 = Nosig','fontSize',fontM,'color',colorBlack);
...
set(hPVCorr(1),'TickDir','out','Box','off','XLim',[0,14],'YLim',[0 1.15],'XTick',[2, 7, 12],'XTickLabel',[{'In-zone','Out-zone','Total'}],'fontSize',fontM);
set(hPVCorr(2),'TickDir','out','Box','off','XLim',[0,14],'YLim',[0 1.15],'XTick',[2, 7, 12],'XTickLabel',[{'In-zone','Out-zone','Total'}],'fontSize',fontM);

print('-painters','-r300','-dtiff',['f_short_suppleXX_PVCorr_InOutTotalZone_8hz_',datestr(now,formatOut),'_v2_kw_5cmBin.tif']);
print('-painters','-r300','-depsc',['f_short_suppleXX_PVCorr_InOutTotalZone_8hz_',datestr(now,formatOut),'_v2_kw_5cmBin.ai']);
close;