...
%% CBP test
% sigArea_PNact = [3 19]; % log-rank test
% sigArea_PNina = [2 14]; % log-rank test
% sigArea_PNnoresp = [5 24]; % log-rank test
sigArea_PNact = [4 19]; % spike based test
sigArea_PNina = [3 24]; % spike based test
sigArea_PNnoresp = [20 26]; % spike based test
...
hold on;
line([sigArea_PNact(1), sigArea_PNact(end)],[15,15],'color',colorRed,'lineWidth', 2, 'lineStyle','-');
hold on;
line([sigArea_PNina(1), sigArea_PNina(end)],[-1.5,-1.5],'color',colorBlue,'lineWidth', 2, 'lineStyle','-');
hold on;
line([sigArea_PNnoresp(1), sigArea_PNnoresp(end)],[-2.5,-2.5],'color',colorDarkGray,'lineWidth', 2, 'lineStyle','-');
...
text(30, 11, ['Activated (n = ',num2str(nAct),')'],'color',colorRed,'fontSize',fontM);
text(30, 8.5, ['Inactivated (n = ',num2str(nIna),')'],'color',colorBlue,'fontSize',fontM);
text(30, 6, ['No response (n = ',num2str(nNoresp),')'],'color',colorBlack,'fontSize',fontM);
xlabel('Time (ms)','fontSize',fontM);
...
%% CBP test
% sigArea_PNact = [2 20]; % log-rank test
% sigArea_PNina = [5 27]; % log-rank test
% sigArea_PNnoresp = [5 38]; % log-rank test
sigArea_PNact = [3 18]; % spike-based test
sigArea_PNina = [-17 28]; % spike-based test
sigArea_PNnoresp = [9 29]; % spike-based test
% sigArea_PNact = statCBP_v2(spikeSmthActStm_Rw,spikeSmthActPre_Rw);
% sigArea_PNina = statCBP_v2(spikeSmthInaStm_Rw,spikeSmthInaPre_Rw);
% sigArea_PNnoresp = statCBP_v2(spikeSmthNorespStm_Rw,spikeSmthNorespPre_Rw);
...
plot(xpt,norm_m_spikeNorespStm,'-o','color',colorBlack,'MarkerFaceColor',colorDarkGray,'markerSize',markerSS,'LineWidth',lineM);
hold on;
line([sigArea_PNact(1), sigArea_PNact(end)],[8.5,8.5],'color',colorRed,'lineWidth', 2, 'lineStyle','-');
hold on;
line([sigArea_PNina(1), sigArea_PNina(end)],[-1.5,-1.5],'color',colorBlue,'lineWidth', 2, 'lineStyle','-');
hold on;
line([sigArea_PNnoresp(1), sigArea_PNnoresp(end)],[-2.5,-2.5],'color',colorDarkGray,'lineWidth', 2, 'lineStyle','-');
...
%%
print('-painters','-r300','-dtiff',['f_short_supple4_track_lightPETH_',datestr(now,formatOut),'.tif']);
print('-painters','-r300','-depsc',['f_short_supple4_track_lightPETH_',datestr(now,formatOut),'.ai']);
close;