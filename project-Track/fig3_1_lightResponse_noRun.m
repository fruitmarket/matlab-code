...
%% Condition
total_noRun = T.taskProb == '100' & T.taskType == 'noRun' & T.peakMap>1;
nTotal_noRun = sum(double(total_noRun));
...
%% Venn diagram
popBaseOnly = total_noRun & (T.pLR_tag<0.05 & ~(T.pLR_modu<0.05));
npopBaseOnly = sum(double(popBaseOnly));
popTrackOnly = total_noRun & (~(T.pLR_tag<0.05) & T.pLR_modu<0.05);
npopTrackOnly = sum(double(popTrackOnly));
popBoth = total_noRun & (T.pLR_tag<0.05 & T.pLR_modu<0.05);
npopBoth = sum(double(popBoth)); 
popNeither = total_noRun & (~(T.pLR_tag<0.05) & ~(T.pLR_modu<0.05));
npopNeither = sum(double(popNeither));

%% Base light response
groupBaseA = total_noRun & T.pLR_tag<0.05 & (T.statDir_tag == 1);
groupBaseB = total_noRun & T.pLR_tag<0.05 & (T.statDir_tag == -1) & ~isnan(T.ina_lastSpk_tag) & ~isnan(T.ina_firstSpk_tag);
groupBaseC = total_noRun & T.pLR_tag<0.05 & (T.statDir_tag == 0);
groupBaseD = total_noRun & ~(T.pLR_tag<0.05) & (T.statDir_tag == 1);
groupBaseE = total_noRun & ~(T.pLR_tag<0.05) & (T.statDir_tag == -1);
groupBaseF = total_noRun & ~(T.pLR_tag<0.05) & (T.statDir_tag == 0);
basePie = [sum(double(groupBaseA)), sum(double(groupBaseB)), sum(double(total_noRun & ~(T.pLR_tag<0.05)))];
labelsBase = {'Activated: ';'Inactivated: ';'Unmodulated: '};

lightBase_pre = T.lighttagPreSpk(total_noRun & T.pLR_tag<0.05);
lightBase_stm = T.lighttagSpk(total_noRun & T.pLR_tag<0.05);
lightBase_post = T.lighttagPostSpk(total_noRun & T.pLR_tag<0.05);

yLimlightBase = max([lightBase_pre; lightBase_stm; lightBase_post])*1.1;

%% Track light response
groupTrackA = total_noRun & T.pLR_modu<0.05 & (T.statDir_modu == 1);
groupTrackB = total_noRun & T.pLR_modu<0.05 & (T.statDir_modu == -1) & ~isnan(T.ina_lastSpk_modu) & ~isnan(T.ina_firstSpk_modu);
groupTrackC = total_noRun & T.pLR_modu<0.05 & (T.statDir_modu == 0);
...
lightTrack_stm = T.lightSpk(total_noRun & T.pLR_modu<0.05);
lightTrack_post = T.lightPostSpk(total_noRun & T.pLR_modu<0.05);
...
hold on;
for iCell = 1:sum(double(total_noRun & T.pLR_tag<0.05))
...
text(3,yLimlightBase*0.8,['n = ',num2str(sum(double(total_noRun & T.pLR_tag<0.05)))],'FontSize',fontL);
...
hold on;
for iCell = 1:sum(double(total_noRun & T.pLR_modu<0.05))
...
text(3,yLimlightTrack*0.8,['n = ',num2str(sum(double(total_noRun & T.pLR_modu<0.05)))],'FontSize',fontL);
ylabel('Spike number','FontSize',fontM);
...
print(gcf,'-painters','-r300','Fig3_1_lightResponse_noRun.ai','-depsc');