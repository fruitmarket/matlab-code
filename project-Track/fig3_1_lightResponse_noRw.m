...
%% Condition
total_noRw = T.taskProb == '100' & T.taskType == 'noRw' & T.peakMap>1;
nTotal_noRw = sum(double(total_noRw));
...
%% Venn diagram
popBaseOnly = total_noRw & (T.pLR_tag<0.05 & ~(T.pLR_modu<0.05));
npopBaseOnly = sum(double(popBaseOnly));
popTrackOnly = total_noRw & (~(T.pLR_tag<0.05) & T.pLR_modu<0.05);
npopTrackOnly = sum(double(popTrackOnly));
popBoth = total_noRw & (T.pLR_tag<0.05 & T.pLR_modu<0.05);
npopBoth = sum(double(popBoth)); 
popNeither = total_noRw & (~(T.pLR_tag<0.05) & ~(T.pLR_modu<0.05));
npopNeither = sum(double(popNeither));

%% Base light response
groupBaseA = total_noRw & T.pLR_tag<0.05 & (T.statDir_tag == 1);
groupBaseB = total_noRw & T.pLR_tag<0.05 & (T.statDir_tag == -1);
groupBaseC = total_noRw & T.pLR_tag<0.05 & (T.statDir_tag == 0);
groupBaseD = total_noRw & ~(T.pLR_tag<0.05) & (T.statDir_tag == 1);
groupBaseE = total_noRw & ~(T.pLR_tag<0.05) & (T.statDir_tag == -1);
groupBaseF = total_noRw & ~(T.pLR_tag<0.05) & (T.statDir_tag == 0);
basePie = [sum(double(groupBaseA)), sum(double(groupBaseB)), sum(double(total_noRw & ~(T.pLR_tag<0.05)))];
labelsBase = {'Activated: ';'Inactivated: ';'Unmodulated: '};

lightBase_pre = T.lighttagPreSpk(total_noRw & T.pLR_tag<0.05);
lightBase_stm = T.lighttagSpk(total_noRw & T.pLR_tag<0.05);
lightBase_post = T.lighttagPostSpk(total_noRw & T.pLR_tag<0.05);

yLimlightBase = max([lightBase_pre; lightBase_stm; lightBase_post])*1.1;

%% Track light response
groupTrackA = total_noRw & T.pLR_modu<0.05 & (T.statDir_modu == 1);
groupTrackB = total_noRw & T.pLR_modu<0.05 & (T.statDir_modu == -1);
groupTrackC = total_noRw & T.pLR_modu<0.05 & (T.statDir_modu == 0);
groupTrackD = total_noRw & ~(T.pLR_modu<0.05) & (T.statDir_modu == 1);
groupTrackE = total_noRw & ~(T.pLR_modu<0.05) & (T.statDir_modu == -1);
groupTrackF = total_noRw & ~(T.pLR_modu<0.05) & (T.statDir_modu == 0);
trackPie = [sum(double(groupTrackA)), sum(double(groupTrackB)), sum(double(total_noRw & ~(T.pLR_modu<0.05)))];
labelsTrack = {'Activated: ';'Inactivated: ';'Unmodulated: '};

lightTrack_pre = T.lightPreSpk(total_noRw & T.pLR_modu<0.05);
lightTrack_stm = T.lightSpk(total_noRw & T.pLR_modu<0.05);
lightTrack_post = T.lightPostSpk(total_noRw & T.pLR_modu<0.05);
...
hold on;
for iCell = 1:sum(double(total_noRw & T.pLR_tag<0.05))
...
text(3,yLimlightBase*0.8,['n = ',num2str(sum(double(total_noRw & T.pLR_tag<0.05)))],'FontSize',fontL);
...
hold on;
for iCell = 1:sum(double(total_noRw & T.pLR_modu<0.05))
...
text(3,yLimlightTrack*0.8,['n = ',num2str(sum(double(total_noRw & T.pLR_modu<0.05)))],'FontSize',fontL);
ylabel('Spike number','FontSize',fontM);
...
print(gcf,'-painters','-r300','Fig3_1_lightResponse_noRw.ai','-depsc');