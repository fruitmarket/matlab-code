...
load cellList_DRun_100.mat

tDRun = T;
idx_PN = tDRun.fr_task <= 10;
idx_IN = tDRun.fr_task > 10;
idx_pSalt = (tDRun.p_saltModu < 0.05 ) | (tDRun.p_saltTag < 0.05);
idx_stb = tDRun.r_Corrhfxhf > 0.9;

% Latency: 1st row:PN, 2nd row: IN
latency(1,1) = nanmean(tDRun.latency_modu(idx_PN & idx_pSalt));
latency(1,2) = nanstd(tDRun.latency_modu(idx_PN & idx_pSalt))/sum(double(idx_PN & idx_pSalt));
latency(2,1) = nanmean(tDRun.latency_modu(idx_IN & idx_pSalt));
latency(2,2) = nanstd(tDRun.latency_modu(idx_IN & idx_pSalt),1)/sum(double(idx_IN & idx_pSalt));
...
yptPnStb = [tDRun.r_Corrhfxhf(idx_PNpSaltStb);tDRun.r_Corrbfxdr(idx_PNpSaltStb);tDRun.r_Corrbfxaft(idx_PNpSaltStb);tDRun.r_Corrdrxaft(idx_PNpSaltStb)];
yptPnUnstb = [tDRun.r_Corrhfxhf(idx_PNpSaltUnstb);tDRun.r_Corrbfxdr(idx_PNpSaltUnstb);tDRun.r_Corrbfxaft(idx_PNpSaltUnstb);tDRun.r_Corrdrxaft(idx_PNpSaltUnstb)];
yptInStb = [tDRun.r_Corrhfxhf(idx_InpSaltStb);tDRun.r_Corrbfxdr(idx_InpSaltStb);tDRun.r_Corrbfxaft(idx_InpSaltStb);tDRun.r_Corrdrxaft(idx_InpSaltStb)];
yptInUnstb = [tDRun.r_Corrhfxhf(idx_InpSaltUnstb);tDRun.r_Corrbfxdr(idx_InpSaltUnstb);tDRun.r_Corrbfxaft(idx_InpSaltUnstb);tDRun.r_Corrdrxaft(idx_InpSaltUnstb)];
...
print(gcf, '-depsc','-r300','fig5_DRun_SaltCorr');
