...
tNolight = T;
idx_PN = tNolight.fr_task <= 10;
idx_IN = tNolight.fr_task > 10;
idx_pSalt = (tNolight.p_saltModu < 0.05 ) | (tNolight.p_saltTag < 0.05);
idx_stb = tNolight.r_Corrhfxhf > 0.9;

% Latency: 1st row:PN, 2nd row: IN
latency(1,1) = nanmean(tNolight.latency_modu(idx_PN & idx_pSalt));
latency(1,2) = nanstd(tNolight.latency_modu(idx_PN & idx_pSalt))/sum(double(idx_PN & idx_pSalt));
latency(2,1) = nanmean(tNolight.latency_modu(idx_IN & idx_pSalt));
latency(2,2) = nanstd(tNolight.latency_modu(idx_IN & idx_pSalt),1)/sum(double(idx_IN & idx_pSalt));
...
yptPnStb = [tNolight.r_Corrhfxhf(idx_PNpSaltStb);tNolight.r_Corrbfxdr(idx_PNpSaltStb);tNolight.r_Corrbfxaft(idx_PNpSaltStb);tNolight.r_Corrdrxaft(idx_PNpSaltStb)];
yptPnUnstb = [tNolight.r_Corrhfxhf(idx_PNpSaltUnstb);tNolight.r_Corrbfxdr(idx_PNpSaltUnstb);tNolight.r_Corrbfxaft(idx_PNpSaltUnstb);tNolight.r_Corrdrxaft(idx_PNpSaltUnstb)];
% yptInStb = [tNolight.r_Corrhfxhf(idx_InpSaltStb);tNolight.r_Corrbfxdr(idx_InpSaltStb);tNolight.r_Corrbfxaft(idx_InpSaltStb);tNolight.r_Corrdrxaft(idx_InpSaltStb)];
yptInUnstb = [tNolight.r_Corrhfxhf(idx_InpSaltUnstb);tNolight.r_Corrbfxdr(idx_InpSaltUnstb);tNolight.r_Corrbfxaft(idx_InpSaltUnstb);tNolight.r_Corrdrxaft(idx_InpSaltUnstb)];
...
print(gcf, '-depsc','-r300','fig5_Nolight_SaltCorr');
