...
load('neuronList_ori_170817.mat');
...
% file example
% plot_Track_multi_v3(T.path(actPC_noRun),T.cellID(actPC_noRun),'C:\Users\Jun\Desktop\noRun_act');
% plot_Track_multi_v3(T.path(inaPC_noRun),T.cellID(inaPC_noRun),'C:\Users\Jun\Desktop\noRun_ina');

%% statistic test 
% [~, p_tPC_preXstm] = ttest2(T.fCorr1D_preXstm(tPC_DRun),T.fCorr1D_preXstm(tPC_noRun));
% [~, p_tPC_preXpost] = ttest2(T.fCorr1D_preXpost(tPC_DRun),T.fCorr1D_preXpost(tPC_noRun));
% [~, p_tPC_stmXpost] = ttest2(T.fCorr1D_stmXpost(tPC_DRun),T.fCorr1D_stmXpost(tPC_noRun));
...
hTotalCorr(1) = axes('Position',axpt(nCol,nRow,1,1,[0.1 0.1 0.855 0.85],midInterval));
...
hold on;
% errorbarJun([2,5,8],m_tPC_noRun_PVcorr,sem_tPC_noRun_PVcorr,eBarLength,eBarWidth,eBarColor);
...
hTotalCorr(2) = axes('Position',axpt(nCol,nRow,2,1,[0.1 0.1 0.855 0.85],midInterval));
...
hTotalCorr(3) = axes('Position',axpt(nCol,nRow,3,1,[0.1 0.1 0.855 0.85],midInterval));
...
hTotalCorr(4) = axes('Position',axpt(nCol,nRow,4,1,[0.1 0.1 0.855 0.85],midInterval));
...
hTotalCorr(5) = axes('Position',axpt(nCol,nRow,5,1,[0.1 0.1 0.855 0.85],midInterval));
...
print('-painters','-r300','-dtiff',[datestr(now,formatOut),'_placeField_PVCorrelation_DRun_noRun.tif']);