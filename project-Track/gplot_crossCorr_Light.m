...
hTotalCorr(3) = axes('Position',axpt(nCol,nRow,1,2,[0.1 0.1 0.85 0.85],midInterval));
% plot([1,2,3],[T.fCorr1D_preXstm(rapidPC), T.fCorr1D_preXpost(rapidPC), T.fCorr1D_stmXpost(rapidPC)],'lineStyle','-','lineWidth',0.5,'color',colorGray);
hold on;
...
hTotalCorr(4) = axes('Position',axpt(nCol,nRow,2,2,[0.1 0.1 0.85 0.85],midInterval));
...
hTotalCorr(5) = axes('Position',axpt(nCol,nRow,3,2,[0.1 0.1 0.85 0.85],midInterval));
...
hTotalCorr(6) = axes('Position',axpt(nCol,nRow,3,1,[0.1 0.1 0.85 0.85],midInterval));
...
hTotalCorr(7) = axes('Position',axpt(nCol,nRow,4,1,[0.1 0.1 0.85 0.85],midInterval));
...
print('-painters','-r300','-dtiff',[datestr(now,formatOut),'_placeFieldCorrelation_DRun.tif']);

%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%% DRw sessions %%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tPC = T.taskType == 'DRw' & T.idxNeurontype == 'PN' & T.idxPeakFR & T.idxPlaceField & T.idxTotalSpikeNum;
actPC = T.taskType == 'DRw' & T.idxNeurontype == 'PN' & T.idxPeakFR & T.idxPlaceField & T.idxTotalSpikeNum & T.idxpLR_Track & T.statDir_TrackN == 1;
directPC = T.taskType == 'DRw' & T.idxNeurontype == 'PN' & T.idxPeakFR & T.idxPlaceField & T.idxTotalSpikeNum & T.idxpLR_Track & T.statDir_TrackN == 1 & Txls.latencyIndex == 'direct';
indirectPC = T.taskType == 'DRw' & T.idxNeurontype == 'PN' & T.idxPeakFR & T.idxPlaceField & T.idxTotalSpikeNum & T.idxpLR_Track & T.statDir_TrackN == 1 & Txls.latencyIndex == 'indirect';
doublePC = T.taskType == 'DRw' & T.idxNeurontype == 'PN' & T.idxPeakFR & T.idxPlaceField & T.idxTotalSpikeNum & T.idxpLR_Track & T.statDir_TrackN == 1 & Txls.latencyIndex == 'double';
inaPC = T.taskType == 'DRw' & T.idxNeurontype == 'PN' & T.idxPeakFR & T.idxPlaceField & T.idxTotalSpikeNum & T.idxpLR_Track & T.statDir_TrackN == -1;
norespPC = T.taskType == 'DRw' & T.idxNeurontype == 'PN' & T.idxPeakFR & T.idxPlaceField & T.idxTotalSpikeNum & ~T.idxpLR_Track;
...
set(hTotalCorr,'TickDir','out','Box','off','XLim',[0.5,4.5],'YLim',[-0.5 5],'XTick',[1,2,3,4],'XTickLabel',[{'preXpre','preXstm','preXpost','stmXpost'}],'fontSize',fontM);
print('-painters','-r300','-dtiff',[datestr(now,formatOut),'_placeFieldCorrelation_DRw.tif']);
close('all');