...
fHandle = figure('PaperUnits','centimeters','PaperPosition',paperSize{1},'Name','Intensity vs Spike P');

hInten(1) = axes('Position',axpt(nCol,nRow,1:2,1:2,[0.1 0.1 0.85 0.85],wideInterval));
plot([1,2,3],[lightPlfm5mw_DRunPn, lightPlfm8mw_DRunPn, lightPlfm10mw_DRunPn],'-o','color',colorDarkGray,'markerSize',markerM,'markerEdgeColor',colorDarkGray,'markerFaceColor',colorLightGray);
hold on;
plot([1,2,3],[m_DRunPN_5mw, m_DRunPN_8mw, m_DRunPN_10mw],'o','color',colorBlack,'markerSize',markerM,'markerEdgeColor',colorBlack,'markerFaceColor',colorBlack);
hold on;
errorbar([1,2,3],[m_DRunPN_5mw, m_DRunPN_8mw, m_DRunPN_10mw],[sem_DRunPN_5mw, sem_DRunPN_8mw, sem_DRunPN_10mw],'o','color',colorBlack,'markerSize',markerM);
...
set(hInten,'Box','off','TickDir','out','XLim',[0,4],'XTick',[1:3],'XTickLabel',{'5mW','8mW','10mW'},'fontSize',fontM);
...
formatOut = 'yymmdd';
% print('-painters','-r300','-dtiff',['plot_plfm_laserIntensity_',datestr(now,formatOut),'.tif']);
print('-painters','-r300','-depsc',['plot_plfm_laserIntensity_',datestr(now,formatOut),'.ai']);
