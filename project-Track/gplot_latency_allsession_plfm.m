...
DRunPN = DRunTN & T.meanFR_task <= cri_meanFR & T.pLR_Plfm2hz<alpha & T.statDir_Plfm2hz == 1;
DRunIN = DRunTN & T.meanFR_task > cri_meanFR & T.pLR_Plfm2hz<alpha & T.statDir_Plfm2hz == 1;

DRwPN = DRwTN & T.meanFR_task <= cri_meanFR & T.pLR_Plfm2hz<alpha & T.statDir_Plfm2hz == 1;
DRwIN = DRwTN & T.meanFR_task > cri_meanFR & T.pLR_Plfm2hz<alpha & T.statDir_Plfm2hz == 1;

noRunPN = noRunTN & T.meanFR_task <= cri_meanFR & T.pLR_Plfm2hz<alpha & T.statDir_Plfm2hz == 1;
noRunIN = noRunTN & T.meanFR_task > cri_meanFR & T.pLR_Plfm2hz<alpha & T.statDir_Plfm2hz == 1;

noRwPN = noRwTN & T.meanFR_task <= cri_meanFR & T.pLR_Plfm2hz<alpha & T.statDir_Plfm2hz == 1;
noRwIN = noRwTN & T.meanFR_task > cri_meanFR & T.pLR_Plfm2hz<alpha & T.statDir_Plfm2hz == 1;
...
latTrack_DRunPN = T.latencyTrack1st(DRunPN & T.pLR_Track<alpha & T.statDir_Track==1);
latTrack_DRunIN = T.latencyTrack1st(DRunIN & T.pLR_Track<alpha & T.statDir_Track==1);

latTrack_DRwPN = T.latencyTrack1st(DRwPN & T.pLR_Track<alpha & T.statDir_Track==1);
latTrack_DRwIN = T.latencyTrack1st(DRwIN & T.pLR_Track<alpha & T.statDir_Track==1);

latTrack_noRunPN = T.latencyTrack1st(noRunPN & T.pLR_Track<alpha & T.statDir_Track==1);
latTrack_noRunIN = T.latencyTrack1st(noRunIN & T.pLR_Track<alpha & T.statDir_Track==1);

latTrack_noRwPN = T.latencyTrack1st(noRwPN & T.pLR_Track<alpha & T.statDir_Track==1);
latTrack_noRwIN = T.latencyTrack1st(noRwIN & T.pLR_Track<alpha & T.statDir_Track==1);
%% plot
nCol = 2;
nRow = 4;
...
text(20,6,['On Platform (n = ',num2str(length(lat_DRunPN)),')'],'fontSize',fontL,'Color',colorBlue);
text(20,4,['On track: (n = ',num2str(length(latTrack_DRunPN)),')'],'fontSize',fontL,'Color',colorRed);
...
text(20,6,['On Platform (n = ',num2str(length(lat_DRunIN)),')'],'fontSize',fontL,'Color',colorBlue);
text(20,4,['On track: (n = ',num2str(length(latTrack_DRunIN)),')'],'fontSize',fontL,'Color',colorRed);
...
text(20,6,['On Platform (n = ',num2str(length(lat_DRwPN)),')'],'fontSize',fontL,'Color',colorBlue);
text(20,4,['On track: (n = ',num2str(length(latTrack_DRwPN)),')'],'fontSize',fontL,'Color',colorRed);
...
text(20,6,['On Platform (n = ',num2str(length(lat_DRwIN)),')'],'fontSize',fontL,'Color',colorBlue);
text(20,4,['On track: (n = ',num2str(length(latTrack_DRwIN)),')'],'fontSize',fontL,'Color',colorRed);
title('IN & DRw','fontSize',fontL,'fontWeight','bold');
xlabel('Latency from light onset (ms)','fontSize',fontL);

hLat(5) = axes('Position',axpt(nCol,nRow,1,3,[0.1 0.1 0.85 0.85],wideInterval));
bar(xpt,histc(lat_noRunPN,0:2:30),1,'FaceColor',colorBlue);
hold on;
text(20,6,['On Platform (n = ',num2str(length(lat_noRunPN)),')'],'fontSize',fontL,'Color',colorBlue);
title('PN & noRun','fontSize',fontL,'fontWeight','bold');
xlabel('Latency from light onset (ms)','fontSize',fontL);

hLat(6) = axes('Position',axpt(nCol,nRow,2,3,[0.1 0.1 0.85 0.85],wideInterval));
bar(xpt,histc(lat_noRunIN,0:2:30),1,'FaceColor',colorBlue);
hold on;
text(20,6,['On Platform (n = ',num2str(length(lat_noRunIN)),')'],'fontSize',fontL,'Color',colorBlue);
title('IN & noRun','fontSize',fontL,'fontWeight','bold');
xlabel('Latency from light onset (ms)','fontSize',fontL);

hLat(7) = axes('Position',axpt(nCol,nRow,1,4,[0.1 0.1 0.85 0.85],wideInterval));
bar(xpt,histc(lat_noRwPN,0:2:30),1,'FaceColor',colorBlue);
hold on;
text(20,6,['On Platform (n = ',num2str(length(lat_noRwPN)),')'],'fontSize',fontL,'Color',colorBlue);
title('PN & noRw','fontSize',fontL,'fontWeight','bold');
xlabel('Latency from light onset (ms)','fontSize',fontL);

hLat(8) = axes('Position',axpt(nCol,nRow,2,4,[0.1 0.1 0.85 0.85],wideInterval));
bar(xpt,histc(lat_noRwIN,0:2:30),1,'FaceColor',colorBlue);
hold on;
text(20,6,['On Platform (n = ',num2str(length(lat_noRwIN)),')'],'fontSize',fontL,'Color',colorBlue);
title('IN & noRw','fontSize',fontL,'fontWeight','bold');
xlabel('Latency from light onset (ms)','fontSize',fontL);

set(hLat,'Box','off','TickDir','out','XLim',[0,30],'XTick',[0:4:30],'YLim',[0,8]);
print('-painters','-r300',['plot_latencySession_plfm_',datestr(date),'.tif'],'-dtiff');
