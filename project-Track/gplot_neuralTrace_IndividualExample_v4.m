...
load('neuronList_ori_171017.mat');
Txls = readtable('neuronList_ori_171017.xlsx');
...
DRun_PN_light = DRunPN & T.pLR_Track<alpha;
DRun_PN_act = DRunPN & T.idxpLR_Track & T.statDir_TrackN == 1;
DRun_PN_ina = DRunPN & T.idxpLR_Track & T.statDir_TrackN == -1;
DRun_PN_no = DRunPN & ~T.idxpLR_Track;
...
%% PETH
pethTime = T.pethtimePsdPre(DRun_PN_light);
xptPETH = pethTime{1};

peth_DRunPN_PRE_light = cell2mat(T.spikePsdPreD(DRun_PN_light));
peth_DRunPN_PRE_act = cell2mat(T.spikePsdPreD(DRun_PN_act));
peth_DRunPN_PRE_ina = cell2mat(T.spikePsdPreD(DRun_PN_ina));
peth_DRunPN_PRE_no = cell2mat(T.spikePsdPreD(DRun_PN_no));

peth_DRunPN_STM_light = cell2mat(T.spikePsdStmD(DRun_PN_light));
peth_DRunPN_STM_act = cell2mat(T.spikePsdStmD(DRun_PN_act));
peth_DRunPN_STM_ina = cell2mat(T.spikePsdStmD(DRun_PN_ina));
peth_DRunPN_STM_no = cell2mat(T.spikePsdStmD(DRun_PN_no));

peth_DRunPN_POST_light = cell2mat(T.spikePsdPostD(DRun_PN_light));
peth_DRunPN_POST_act = cell2mat(T.spikePsdPostD(DRun_PN_act));
peth_DRunPN_POST_ina = cell2mat(T.spikePsdPostD(DRun_PN_ina));
peth_DRunPN_POST_no = cell2mat(T.spikePsdPostD(DRun_PN_no));
...
for iBin = 1:nBin
    [p_light1(iBin,1)] = ranksum(peth_DRunPN_PRE_light(:,iBin),peth_DRunPN_STM_light(:,iBin));
    [p_act1(iBin,1)] = ranksum(peth_DRunPN_PRE_act(:,iBin),peth_DRunPN_STM_act(:,iBin));
    [p_ina1(iBin,1)] = ranksum(peth_DRunPN_PRE_ina(:,iBin),peth_DRunPN_STM_ina(:,iBin));
    [~,p_no1(iBin,1)] = ttest(peth_DRunPN_PRE_no(:,iBin),peth_DRunPN_STM_no(:,iBin));
    [p_light2(iBin,1)] = ranksum(peth_DRunPN_POST_light(:,iBin),peth_DRunPN_STM_light(:,iBin));
    [p_act2(iBin,1)] = ranksum(peth_DRunPN_POST_act(:,iBin),peth_DRunPN_STM_act(:,iBin));
    [p_ina2(iBin,1)] = ranksum(peth_DRunPN_POST_ina(:,iBin),peth_DRunPN_STM_ina(:,iBin));
    [~,p_no2(iBin,1)] = ttest(peth_DRunPN_POST_no(:,iBin),peth_DRunPN_STM_no(:,iBin));
end

loci_p_light1 = find(p_light1<0.05); % shift to - 22ms
loci_p_act1 = find(p_act1<0.05); 
loci_p_ina1 = find(p_ina1<0.05);
loci_p_no1 = find(p_no1<0.05);

loci_p_light2 = find(p_light2<0.05); % shift to - 22ms
loci_p_act2 = find(p_act2<0.05); 
loci_p_ina2 = find(p_ina2<0.05);
loci_p_no2 = find(p_no2<0.05);
...
tempY_lower = repmat(m_DRunPN_PRE_light90(1:123),2,1);
tempY_lower = tempY_lower(:);
tempY_upper = repmat(m_DRunPN_PRE_light110(1:123),2,1);
tempY_upper = tempY_upper(:);
yptFill_PRE_light = [tempY_lower; tempY_upper];

tempY_lower = repmat(m_DRunPN_PRE_act90(1:123),2,1);
tempY_lower = tempY_lower(:);
tempY_upper = repmat(m_DRunPN_PRE_act110(1:123),2,1);
tempY_upper = tempY_upper(:);
yptFill_PRE_act = [tempY_lower; tempY_upper];

tempY_lower = repmat(m_DRunPN_PRE_ina90(1:123),2,1);
tempY_lower = tempY_lower(:);
tempY_upper = repmat(m_DRunPN_PRE_ina110(1:123),2,1);
tempY_upper = tempY_upper(:);
yptFill_PRE_ina = [tempY_lower; tempY_upper];

tempY_lower = repmat(m_DRunPN_PRE_no90(1:123),2,1);
tempY_lower = tempY_lower(:);
tempY_upper = repmat(m_DRunPN_PRE_no110(1:123),2,1);
tempY_upper = tempY_upper(:);
yptFill_PRE_no = [tempY_lower; tempY_upper];

tempY_lower = repmat(m_DRunPN_POST_light90(1:123),2,1);
tempY_lower = tempY_lower(:);
tempY_upper = repmat(m_DRunPN_POST_light110(1:123),2,1);
tempY_upper = tempY_upper(:);
yptFill_POST_light = [tempY_lower; tempY_upper];

tempY_lower = repmat(m_DRunPN_POST_act90(1:123),2,1);
tempY_lower = tempY_lower(:);
tempY_upper = repmat(m_DRunPN_POST_act110(1:123),2,1);
tempY_upper = tempY_upper(:);
yptFill_POST_act = [tempY_lower; tempY_upper];

tempY_lower = repmat(m_DRunPN_POST_ina90(1:123),2,1);
tempY_lower = tempY_lower(:);
tempY_upper = repmat(m_DRunPN_POST_ina110(1:123),2,1);
tempY_upper = tempY_upper(:);
yptFill_POST_ina = [tempY_lower; tempY_upper];

tempY_lower = repmat(m_DRunPN_POST_no90(1:123),2,1);
tempY_lower = tempY_lower(:);
tempY_upper = repmat(m_DRunPN_POST_no110(1:123),2,1);
tempY_upper = tempY_upper(:);
...
% stairs(xpt, m_DRunPN_PRE_light90(1:123),'LineStyle',':','color',colorRed);
% [xx, yy] = stairs(xpt,m_DRunPN_PRE_light90(1:123));
% patch([xx],[yy],colorGray);
hold on;
stairs(xpt, m_DRunPN_PRE_light(1:123),'color',colorBlack,'lineWidth',1.2);
hold on;
stairs(xpt, m_DRunPN_STM_light(1:123),'color',colorBlue,'lineWidth',1.2);
hold on;
...
hold on;
stairs(xpt, m_DRunPN_POST_light(1:123),'color',colorBlack,'lineWidth',1.2);
hold on;
stairs(xpt, m_DRunPN_STM_light(1:123),'color',colorBlue,'lineWidth',1.2);
hold on;
...
stairs(xpt, m_DRunPN_PRE_act110(1:123),'LineStyle','-','color',colorRed);
hold on;
stairs(xpt, m_DRunPN_PRE_act(1:123),'color',colorBlack,'lineWidth',1.2);
hold on;
stairs(xpt, m_DRunPN_STM_act(1:123),'color',colorBlue,'lineWidth',1.2);
hold on;
...
stairs(xpt, m_DRunPN_POST_act110(1:123),'LineStyle','-','color',colorRed);
hold on;
stairs(xpt, m_DRunPN_POST_act(1:123),'color',colorBlack,'lineWidth',1.2);
hold on;
stairs(xpt, m_DRunPN_STM_act(1:123),'color',colorBlue,'lineWidth',1.2);
hold on;
...
stairs(xpt, m_DRunPN_PRE_ina90(1:123),'LineStyle','-','color',colorRed);
hold on;
stairs(xpt, m_DRunPN_PRE_ina(1:123),'color',colorBlack,'lineWidth',1.2);
hold on;
stairs(xpt, m_DRunPN_STM_ina(1:123),'color',colorBlue,'lineWidth',1.2);
hold on;
...
stairs(xpt, m_DRunPN_POST_ina90(1:123),'LineStyle','-','color',colorRed);
hold on;
stairs(xpt, m_DRunPN_POST_ina(1:123),'color',colorBlack,'lineWidth',1.2);
hold on;
stairs(xpt, m_DRunPN_STM_ina(1:123),'color',colorBlue,'lineWidth',1.2);
hold on;
...
stairs(xpt, m_DRunPN_PRE_no(1:123),'color',colorBlack,'lineWidth',1.2);
hold on;
stairs(xpt, m_DRunPN_STM_no(1:123),'color',colorBlue,'lineWidth',1.2);
hold on;
...
stairs(xpt, m_DRunPN_POST_no(1:123),'color',colorBlack,'lineWidth',1.2);
hold on;
stairs(xpt, m_DRunPN_STM_no(1:123),'color',colorBlue,'lineWidth',1.2);
hold on;
