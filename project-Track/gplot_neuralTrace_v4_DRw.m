function gplot_neuralTrace_v4_DRw
...
spikePRE = spikePsdPreD(DRw_PN_ina,:);
spikeSTM = spikePsdStmD(DRw_PN_ina,:);
spikePOST = spikePsdPostD(DRw_PN_ina,:);
...
ab_sum_normF_SpikePOST = nansum(normF_SpikePOST(peri,:),1);

ab_mean_normF_SpikePRE = mean(normF_SpikePRE(peri,:),1);
ab_mean_normF_SpikeSTM = mean(normF_SpikeSTM(peri,:),1);
ab_mean_normF_SpikePOST = nanmean(normF_SpikePOST(peri,:),1);
...
%% reference figures
% fileList = T.path(DRw_PN_ina);
% cellID = T.cellID(DRw_PN_ina);
...
print('-painters','-r300','-dtiff',[datestr(now,formatOut),'_neuralTrace_v4_DRw','.tif']);
