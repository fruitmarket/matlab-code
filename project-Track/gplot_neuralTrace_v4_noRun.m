function gplot_neuralTrace_v4_noRun
...
noRun_PN_ina = T.taskType == 'noRun' & T.idxNeurontype == 'PN' & T.idxPeakFR & T.idxPlaceField & T.idxTotalSpikeNum & T.idxpLR_Track & T.statDir_TrackN == -1;

peri = ~logical(T.idxOverLapLengthSTM(noRun_PN_ina)); % index of overlap
center = logical(T.idxOverLapLengthSTM(noRun_PN_ina)); % index of overlap
...
spikePRE = spikePsdPreD(noRun_PN_ina,:);
spikeSTM = spikePsdStmD(noRun_PN_ina,:);
spikePOST = spikePsdPostD(noRun_PN_ina,:);
...
sum_spikeSmthPOST = nansum(spikeSmthPOST./repmat(mean(spikeSmthPOST(:,1:10),2),1,120),1);
...
mean_spikeSmthPOST = nanmean(spikeSmthPOST./repmat(mean(spikeSmthPOST(:,1:10),2),1,120),1);
...
neuralTrace_POST = sqrt(sum(norm_NT_POST([1,2],1)));
...
ab_sum_normF_SpikePOST = sum(normF_SpikePOST(peri(1:2),:),1);

ab_mean_normF_SpikePRE = mean(normF_SpikePRE(peri,:),1);
ab_mean_normF_SpikeSTM = mean(normF_SpikeSTM(peri,:),1);
ab_mean_normF_SpikePOST = mean(normF_SpikePOST(peri(1:2),:),1);
...
ab_sum_spikeSmthPOST = sum(spikeSmthPOST(peri(1:2),:)./repmat(mean(spikeSmthPOST(peri(1:2),1:10),2),1,120),1);
...
ab_mean_spikeSmthPOST = mean(spikeSmthPOST(peri(1:2),:)./repmat(mean(spikeSmthPOST(peri(1:2),1:10),2),1,120),1);

ab_neuralTrace_PRE = sqrt(sum(norm_NT_PRE(peri,:),1));
ab_neuralTrace_STM = sqrt(sum(norm_NT_STM(peri,:),1));
ab_neuralTrace_POST = sqrt(sum(norm_NT_POST(peri(1:2),:),1));
...
%% reference figures
% fileList = T.path(noRun_PN_ina);
% cellID = T.cellID(noRun_PN_ina);
...
text(-5,15,'Raw normalized spike mean','fontSize',fontM);
...
text(-5,60,'Smoothed normalized spike sum','fontSize',fontM);
...
text(-5,13,'Smoothed normalized spike mean','fontSize',fontM);
...
text(-5,14,'Neural trace','fontSize',fontM);
...
set(hRaw([2,4,6]),'Box','off','TickDir','out','XLim',[-12,111],'XTick',[-10:10:40,100],'YLim',[0 20],'fontSize',fontM);
set(hSmth([1,3,5]),'Box','off','TickDir','out','XLim',[-10,109],'XTick',[-10:10:40,100],'YLim',[0 70],'fontSize',fontM);
set(hSmth([2,4,6]),'Box','off','TickDir','out','XLim',[-10,109],'XTick',[-10:10:40,100],'YLim',[0 15],'fontSize',fontM);
set(hTrace,'Box','off','TickDir','out','XLim',[-10,109],'YLim',[0, 15],'XTick',[-10:10:40,100],'fontSize',fontM);

print('-painters','-r300','-dtiff',[datestr(now,formatOut),'_neuralTrace_v4_noRun','.tif']);
