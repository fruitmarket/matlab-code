function gplot_neuralTrace_v4_noRw
...
noRw_PN_ina = T.taskType == 'noRw' & T.idxNeurontype == 'PN' & T.idxPeakFR & T.idxPlaceField & T.idxTotalSpikeNum & T.idxpLR_Track & T.statDir_TrackN == -1;

peri = ~logical(T.idxOverLapLengthSTM(noRw_PN_ina)); % index of overlap
center = logical(T.idxOverLapLengthSTM(noRw_PN_ina)); % index of overlap
...
spikePRE = spikePsdPreD(noRw_PN_ina,:);
spikeSTM = spikePsdStmD(noRw_PN_ina,:);
spikePOST = spikePsdPostD(noRw_PN_ina,:);
...
%% reference figures
% fileList = T.path(noRw_PN_ina);
% cellID = T.cellID(noRw_PN_ina);
...
print('-painters','-r300','-dtiff',[datestr(now,formatOut),'_neuralTrace_v4_noRw','.tif']);
