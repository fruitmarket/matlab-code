...
cSpkpvr = 1.1;
alpha = 0.01;
areaDRun = [5/6 8/6]*pi*20; % 6-9 o'clock
areaDRw = [3/2 5/3]*pi*20; % 10-11 o'clock
areaRw1 = [3/6 4/6]*pi*20; % 4-5 o'clock
areaRw2 = [9/6 10/6]*pi*20; % 10-11 o'clock
errorPosi = 5;
correctY = 0.5;
lightBand = 3;
...
fHandle = figure('PaperUnits','centimeters','PaperPosition',[0, 0, 15, 20]);
% Figure PN
...
for iMark = 1:length(lightResp_plfm_idx_DRunPN)
    fill([132 133 133 132],[lightResp_plfm_idx_DRunPN(iMark), lightResp_plfm_idx_DRunPN(iMark), lightResp_plfm_idx_DRunPN(iMark)+1, lightResp_plfm_idx_DRunPN(iMark)+1],colorBlack,'EdgeColor','none');
end
...
for iMark = 1:length(lightResp_idx_DRwPN)
    fill([126, 127, 127, 126],[lightResp_idx_DRwPN(iMark), lightResp_idx_DRwPN(iMark), lightResp_idx_DRwPN(iMark)+1, lightResp_idx_DRwPN(iMark)+1],colorGreen,'EdgeColor','none');
end
for iMark = 1:length(lightAct_idx_DRwPN)
    fill([128, 129, 129, 128],[lightAct_idx_DRwPN(iMark), lightAct_idx_DRwPN(iMark), lightAct_idx_DRwPN(iMark)+1, lightAct_idx_DRwPN(iMark)+1],colorBlue,'EdgeColor','none');
end
for iMark = 1:length(lightIna_idx_DRwPN)
    fill([130, 131, 131, 130],[lightIna_idx_DRwPN(iMark), lightIna_idx_DRwPN(iMark), lightIna_idx_DRwPN(iMark)+1, lightIna_idx_DRwPN(iMark)+1],colorRed,'EdgeColor','none');
end
for iMark = 1:length(lightResp_plfm_idx_DRwPN)
    fill([132 133 133 132],[lightResp_plfm_idx_DRwPN(iMark), lightResp_plfm_idx_DRwPN(iMark), lightResp_plfm_idx_DRwPN(iMark)+1, lightResp_plfm_idx_DRwPN(iMark)+1],colorBlack,'EdgeColor','none');
end

xlabel('Position (cm)','fontSize',fontL);
ylabel('Cell ID','fontSize',fontL);
text(-40,-22,['Green: Light Resp (n = ',num2str(length(lightResp_idx_DRwPN)),')'],'color',colorGreen,'fontSize',fontL);
text(50,-22,['Blue: Light Act (n = ',num2str(length(lightAct_idx_DRwPN)),')'],'color',colorBlue,'fontSize',fontL);
text(-40,-24,['Red: Light Ina (n = ',num2str(length(lightIna_idx_DRwPN)),')'],'color',colorRed,'fontSize',fontL);
text(50,-24,['Yellow: Light Resp (Plfm 2hz) (n = ',num2str(length(lightResp_plfm_idx_DRwPN)),')'],'color',colorBlack,'fontSize',fontL);
...
for iMark = 1:length(lightResp_idx_DRunIN)
    fill([126, 127, 127, 126],[lightResp_idx_DRunIN(iMark), lightResp_idx_DRunIN(iMark), lightResp_idx_DRunIN(iMark)+1, lightResp_idx_DRunIN(iMark)+1],colorGreen,'EdgeColor','none');
end
for iMark = 1:length(lightAct_idx_DRunIN)
    fill([128, 129, 129, 128],[lightAct_idx_DRunIN(iMark), lightAct_idx_DRunIN(iMark), lightAct_idx_DRunIN(iMark)+1, lightAct_idx_DRunIN(iMark)+1],colorBlue,'EdgeColor','none');
end
for iMark = 1:length(lightIna_idx_DRunIN)
    fill([130, 131, 131, 130],[lightIna_idx_DRunIN(iMark), lightIna_idx_DRunIN(iMark), lightIna_idx_DRunIN(iMark)+1, lightIna_idx_DRunIN(iMark)+1],colorRed,'EdgeColor','none');
end
for iMark = 1:length(lightResp_plfm_idx_DRunIN)
    fill([132 133 133 132],[lightResp_plfm_idx_DRunIN(iMark), lightResp_plfm_idx_DRunIN(iMark), lightResp_plfm_idx_DRunIN(iMark)+1, lightResp_plfm_idx_DRunIN(iMark)+1],colorBlack,'EdgeColor','none');
end
...
hold on;
pPatch(2) = patch([areaRw2(1)-errorPosi, areaRw2(2)-errorPosi, areaRw2(2)-errorPosi, areaRw2(1)-errorPosi],[-lightBand, -lightBand, barH, barH],colorBlue);
xlabel('Position (cm)','fontSize',fontL);
ylabel('Cell ID','fontSize',fontL);

for iMark = 1:length(lightResp_idx_DRwIN)
    fill([126, 127, 127, 126],[lightResp_idx_DRwIN(iMark), lightResp_idx_DRwIN(iMark), lightResp_idx_DRwIN(iMark)+1, lightResp_idx_DRwIN(iMark)+1],colorGreen,'EdgeColor','none');
end
for iMark = 1:length(lightAct_idx_DRwIN)
    fill([128, 129, 129, 128],[lightAct_idx_DRwIN(iMark), lightAct_idx_DRwIN(iMark), lightAct_idx_DRwIN(iMark)+1, lightAct_idx_DRwIN(iMark)+1],colorBlue,'EdgeColor','none');
end
for iMark = 1:length(lightIna_idx_DRwIN)
    fill([130, 131, 131, 130],[lightIna_idx_DRwIN(iMark), lightIna_idx_DRwIN(iMark), lightIna_idx_DRwIN(iMark)+1, lightIna_idx_DRwIN(iMark)+1],colorRed,'EdgeColor','none');
end
for iMark = 1:length(lightResp_plfm_idx_DRwIN)
    fill([132 133 133 132],[lightResp_plfm_idx_DRwIN(iMark), lightResp_plfm_idx_DRwIN(iMark), lightResp_plfm_idx_DRwIN(iMark)+1, lightResp_plfm_idx_DRwIN(iMark)+1],colorBlack,'EdgeColor','none');
end
...
formatOut = 'yymmdd';
print('-painters','-r300','-dtiff',['gplot_placeField_1stSess_',datestr(now,formatOut),'.tif']);