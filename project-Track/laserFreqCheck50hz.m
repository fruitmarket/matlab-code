...
for iCell = 1:nCell
 ...
 %% Platform 2hz & 8hz
    if isempty(lightTime.Plfm2hz) | isempty(lightTime.Plfm50hz)
     ...
     nPlfmLight8hz = length(lightTime.Plfm50hz);
     spikeTimePlfm50hz = spikeWin(tData{iCell},lightTime.Plfm50hz,winCri);
     lightPlfmSpk50hz = sum(cellfun(@length,spikeTimePlfm50hz))/nPlfmLight8hz;    
    end

%% Track 'Modi session' only
    if isempty(lightTime.Track2hz) | isemtpy(lightTime.Track50hz)
        ...
        nTrackLight50hz = length(lightTime.Track50hz);
        spikeTimeTrack50hz = spikeWin(tData{iCell},lightTime.Track50hz,winCri);
        lightTrackSpk50hz = sum(cellfun(@length, spikeTimeTrack50hz))/nTrackLight50hz;
    end

%%
save([cellName,'.mat'],'lightPlfmSpk2hz8mw','lightPlfmSpk50hz','lightTrackSpk2hz8mw','lightTrackSpk50hz','-append');
end
