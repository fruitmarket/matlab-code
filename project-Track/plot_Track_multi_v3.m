...
% Light response spike probability 
    ...
    text(0.5,0.6,['Pre: ',num2str(m_inzoneSpike(1))],'fontSize',fontM);
    text(0.5,0.4,['Stm: ',num2str(m_inzoneSpike(2))],'fontSize',fontM);
    text(0.5,0.2,['Post: ',num2str(m_inzoneSpike(3))],'fontSize',fontM);
...
% Track light response raster plot (aligned on pseudo light - light - pseudo light)
    ...
    set(hTrackLight(1),'Box','off','TickDir','out','LineWidth',lineS,'FontSize',fontM,'XLim',[0 125],'XTick',[],'YLim',[0, sum([length(psdlightPre),length(lightTime.Track8hz),length(psdlightPost)])],'YTick',[0,length(psdlightPre),sum([length(psdlightPre),length(lightTime.Track8hz)]),sum([length(psdlightPre),length(lightTime.Track8hz),length(psdlightPost)])]);
    set(hTrackLight(2),'Box','off','TickDir','out','LineWidth',lineS,'FontSize',fontM,'XLim',[0,125],'XTick',[0,10,30,50,100,125],'YLim',[0, max(ylimpeth)]);
...
% Zone spike analysis
    yLimTotal = max([sum_totalSpike(1,1), sum_totalSpike(2,1), sum_totalSpike(3,1)])*1.2+0.01;
    yLimInzone = max([sum_inzoneSpike(1,1), sum_inzoneSpike(2,1), sum_inzoneSpike(3,1)])*1.2+0.01;
    yLimOutzone = max([sum_outzoneSpike(1,1), sum_outzoneSpike(2,1), sum_outzoneSpike(3,1)])*1.2+0.01;
    hSpike(1) = axes('Position',axpt(6,2,1:2,1,axpt(nCol,nRow,1:5,10:11,[0.10 0.08 0.85 0.85],midInterval),midInterval));
        plot([1,2,3],[sum_totalSpike(1,1), sum_totalSpike(2,1), sum_totalSpike(3,1)],'-o','color',colorBlack,'MarkerEdgeColor',colorBlack,'MarkerFaceColor',colorGray,'MarkerSize',markerM);
        if p_ttest(1,3) < 0.05
            line([1.1,1.9],[yLimTotal yLimTotal]/1.2*1.05,'lineStyle','-','lineWidth',lineM,'color',colorBlack);
            text(1.4,yLimTotal/1.2*1.05,'*','fontSize',fontL,'color',colorRed);
        end
        if p_ttest(2,3) < 0.05
            line([1,3],[yLimTotal yLimTotal]/1.2*1.1,'lineStyle','-','lineWidth',lineM,'color',colorBlack);
            text(1.9,yLimTotal/1.2*1.1,'*','fontSize',fontL,'color',colorRed);
        end
        if p_ttest(3,3) < 0.05
            line([2.1,2.9],[yLimTotal yLimTotal]/1.2*1.05,'lineStyle','-','lineWidth',lineM,'color',colorBlack);
            text(2.4,yLimTotal/1.2*1.05,'*','fontSize',fontL,'color',colorRed);
        end
    ...
    hSpike(2) = axes('Position',axpt(6,2,3:4,1,axpt(nCol,nRow,1:5,10:11,[0.10 0.08 0.85 0.85],midInterval),midInterval));
        plot([1,2,3],[sum_inzoneSpike(1,1), sum_inzoneSpike(2,1), sum_inzoneSpike(3,1)],'-o','color',colorBlack,'MarkerEdgeColor',colorBlack,'MarkerFaceColor',colorGray,'MarkerSize',markerM);
        if p_ttest(1,1) < 0.05
            line([1.1,1.9],[yLimInzone yLimInzone]/1.2*1.05,'lineStyle','-','lineWidth',lineM,'color',colorBlack);
            text(1.4,yLimInzone/1.2*1.05,'*','fontSize',fontL,'color',colorRed);
        end
        if p_ttest(2,1) < 0.05
            line([1,3],[yLimInzone yLimInzone]/1.2*1.1,'lineStyle','-','lineWidth',lineM,'color',colorBlack);
            text(1.9,yLimInzone/1.2*1.1,'*','fontSize',fontL,'color',colorRed);
        end
        if p_ttest(3,1) < 0.05
            line([2.1,2.9],[yLimInzone yLimInzone]/1.2*1.05,'lineStyle','-','lineWidth',lineM,'color',colorBlack);
            text(2.4,yLimInzone/1.2*1.05,'*','fontSize',fontL,'color',colorRed);
        end
    ...
    hSpike(3) = axes('Position',axpt(6,2,5:6,1,axpt(nCol,nRow,1:5,10:11,[0.10 0.08 0.85 0.85],midInterval),midInterval));
        plot([1,2,3],[sum_outzoneSpike(1,1), sum_outzoneSpike(2,1), sum_outzoneSpike(3,1)],'-o','color',colorBlack,'MarkerEdgeColor',colorBlack,'MarkerFaceColor',colorGray,'MarkerSize',markerM);
        if p_ttest(1,2) < 0.05
            line([1.1,1.9],[yLimOutzone yLimOutzone]/1.2*1.05,'lineStyle','-','lineWidth',lineM,'color',colorBlack);
            text(1.4,yLimOutzone/1.2*1.05,'*','fontSize',fontL,'color',colorRed);
        end
        if p_ttest(2,2) < 0.05
            line([1,3],[yLimOutzone yLimOutzone]/1.2*1.1,'lineStyle','-','lineWidth',lineM,'color',colorBlack);
            text(1.9,yLimOutzone/1.2*1.1,'*','fontSize',fontL,'color',colorRed);
        end
        if p_ttest(3,2) < 0.05
            line([2.1,2.9],[yLimOutzone yLimOutzone]/1.2*1.05,'lineStyle','-','lineWidth',lineM,'color',colorBlack);
            text(2.4,yLimOutzone/1.2*1.05,'*','fontSize',fontL,'color',colorRed);
        end
...
% LFP analysis (Track 8hz)
    ...
    text(xLimCSC(1), yLimCSC_Track8hz(2),'LFP (Track 8hz)','fontSize',fontM);
    
    set(hLFP(1),'Xlim',xLimCSC,'XTick',[],'YLim',yLimCSC_Plfm2hz,'fontSize',fontM);
    set(hLFP(2),'Xlim',xLimCSC,'XTick',[],'YLim',yLimCSC_Track8hz,'fontSize',fontM);
