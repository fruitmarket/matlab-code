...
timeStep =  1; % 1ms
firingRate = 40; % 4hz
durationS = 1000*10^3; % 100 sec simulation
time = 0:timeStep:durationS;
timeLightOn = (0:125:durationS);

win = [-25 100];
binSize = 2;
resolution = 10;
vt = rand(size(time))*1000;

spikes = (firingRate*timeStep) > vt;
spikeT = time(spikes);

spikeTimeLight = spikeWin(spikeT, timeLightOn, win);
[xpt, ypt, pethTime, peth, pethconv, pethconvZ] = rasterPETH(spikeTimeLight,true(size(timeLightOn)),win,binSize,resolution);
disp('ok');
