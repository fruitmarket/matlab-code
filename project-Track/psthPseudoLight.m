function psthPseudoLight
% psthPseudoLight generates Raster & PETH aligned on Pseudolight(Pre) - light(Stm) - Pseudolight(Post)
% 
%
%
...
binSize = 5;
resolution = 10;
win = [-1,1]*50; % window for binning (100ms)
...
for iCell = 1:nCell
    ...
    load('Events.mat','psdlightPre','psdlightPost','lightTime');
    lightTrackOnset = [psdlightPre;lightTime.Modu;psdlightPost];
    A = [1,0,0];
    B = [0,1,0];
    C = [0,0,1];
    trialIndex = logical([repmat(A,length(psdlightPre),1);repmat(B,length(lightTime.Modu),1);repmat(C,length(psdlightPost))];
        
    spikeData = tData{iCell};
    spikeTime = spikeWin(spikeData,lightTrackOnset,win(1,:));
    [xptLightTrack, yptLightTrack, pethTimeLightTrack, pethBarLightTrack, pethConvLightTrack, pethConvzLightTrack] = rasterPETH(spikeTime,trialIndex,win,binSize,resolution,1);
    
    save([cellName,'.mat'],'xptLightTrack','yptLightTrack','pethTimeLightTrack','pethBarLightTrack','pethConvLightTrack','pethConvzLightTrack','-append');
end
