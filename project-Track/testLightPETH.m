...
for iCell = 1:nCell
    ...
    nLight = length(lightOnset);
    newLight = zeros(nLight,1);
    for iLight = 1:nLight
        [~,lightIdx] = min(abs(lightOnset(iLight)-timeTrack));
        newLight(iLight) = timeTrack(lightIdx);
    end
    lightPositionIdx = zeros(nLight,1);
    for iLight = nLight
        lightPositionIdx(iLight) = find((timeTrack == newLight(iLight)));
    end
