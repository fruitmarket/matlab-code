...
populPass = ((cellfun(@(x) min(mean(x(31:60))),T.sensorMeanFR_DRun)) >= mean_mfr_zone) & (T.pLR_Track<alpha);
populAct = populPass & (T.statDir_Track == 1);
populIna = populPass & (T.statDir_Track == -1);
populNoresp = ((cellfun(@(x) min(mean(x(31:60))),T.sensorMeanFR_DRun)) >= mean_mfr_zone) & (T.pLR_Track>=alpha);

populPass = double(populPass(DRunPN));
populAct = populAct(DRunPN);
populIna = populIna(DRunPN);
populNoresp = double(populNoresp(DRunPN));
...
normTrackFR_DRunPN(:,8) = num2cell(populNoresp);
