...
p_sr_stim(1) = signrank(squeeze(timeBlock_zone_nov(3,3,:)),squeeze(timeBlock_zone_nov(3,1,:)));
p_sr_stim(2) = signrank(squeeze(timeBlock_zone_fam(2,3,:)),squeeze(timeBlock_zone_fam(2,1,:)));
...
hTimeLap = axes('Position',axpt(6,6,1:3,1:5,axpt(nCol,nRow,1,1,[],wideInterval),wideInterval));
    hPatch(1) = patch([xptTrial,fliplr(xptTrial)],[m_timeLap_nov(31:120)'-sem_timeLap_nov(31:120)',fliplr(m_timeLap_nov(31:120)'+sem_timeLap_nov(31:120)')],colorDarkGray,'edgeColor','none');
    hold on;
    hPatch(2) = patch([xptTrial,fliplr(xptTrial)],[m_timeLap_fam'-sem_timeLap_fam',fliplr(m_timeLap_fam'+sem_timeLap_fam')],colorLightGray,'edgecolor',colorDarkGray);
    hold on;
    hPatch(3) = patch([30 60 60 30],[0.3 0.3 45*0.05 45*0.05],colorLightBlue,'edgeColor','none');
    hPlot(1) = plot(xptTrial,m_timeLap_nov(31:120),'lineStyle','-','lineWidth',1,'color',colorBlack);
    hold on;
    hPlot(2) = plot(xptTrial,m_timeLap_fam,'lineStyle','-','lineWidth',1,'color',colorBlack);
...
% panel 2 (block comparison)
hTimeLapBlock = axes('Position',axpt(7,3,1:3,1:2,axpt(nCol,nRow,1,2,[],wideInterval),wideInterval));
...
% panel 3
hTimeUnstimBar = axes('Position',axpt(7,3,4:5,1:2,axpt(nCol,nRow,1,2,[],wideInterval),wideInterval));
    plot(xptPRE,[timeBlock_nov(:,3),timeBlock_fam(:,3)],'lineStyle','-','marker','none','color',colorDarkGray);
    hold on;
    errorbarJun(xptPRE,[m_eachBlock_nov(3),m_eachBlock_fam(3)],[sem_eachBlock_nov(3), sem_eachBlock_fam(3)],eBarLength-0.1,eBarWidth,colorBlack);
    hold on;
    ...
    text(0,-7,['KW-test: ',num2str(p_sr_sup(1))],'fontSize',fontM);
    text(1.5,23,'*','color',colorRed,'fontSize',fontL);
    ylabel('Time (min)','fontSize',fontM);

    set(hTimeUnstimBar,'Box','off','TickDir','out','XLim',[0.5 2.5],'XTick',[1 2],'XTickLabel',{'Novel','Familiar'},'YLim',[0 25],'YTick',[0:5:25],'fontSize',fontM);
    
% panel 4
hTimeStimBar(1) = axes('Position',axpt(7,3,1:2,1:2,axpt(nCol,nRow,1,3,[],wideInterval),wideInterval));
    plot([1,3],[squeeze(timeBlock_zone_nov(3,3,:)),squeeze(timeBlock_zone_nov(3,1,:))],'lineStyle','-','marker','none','color',colorDarkGray);
    hold on;
    errorbarJun([1,3],[m_timeBlock_zone_nov(3,3),m_timeBlock_zone_nov(3,1)],[sem_timeBlock_zone_nov(3,3), sem_timeBlock_zone_nov(3,1)],eBarLength-0.15,eBarWidth,colorBlack);
    hold on;
    plot([1,3],[m_timeBlock_zone_nov(3,3),m_timeBlock_zone_nov(3,1)],'lineStyle','-','marker','none','color',colorBlack);
    hold on;
    ...
    hold on;
    plot(3,m_timeBlock_zone_nov(3,1),'lineStyle','none','marker','o','markerSize',markerS,'color',colorBlack,'markerFacecolor',colorDarkGray);
    text(0,-4,['SR test: ',num2str(p_sr_stim(1),3)],'fontSize',fontM);
    ylabel('Time (min)','fontSize',fontM);
    
 hTimeStimBar(2) = axes('Position',axpt(7,3,3:4,1:2,axpt(nCol,nRow,1,3,[],wideInterval),wideInterval));
    plot([1,3],[squeeze(timeBlock_zone_fam(2,3,:)),squeeze(timeBlock_zone_fam(2,1,:))],'lineStyle','-','marker','none','color',colorDarkGray);
    hold on;
    errorbarJun([1,3],[m_timeBlock_zone_fam(2,3),m_timeBlock_zone_fam(2,1)],[sem_timeBlock_zone_fam(2,3), sem_timeBlock_zone_fam(2,1)],eBarLength-0.15,eBarWidth,colorBlack);
    hold on;
    plot([1,3],[m_timeBlock_zone_fam(2,3),m_timeBlock_zone_fam(2,1)],'lineStyle','-','marker','none','color',colorBlack);
    hold on;
    plot(1,m_timeBlock_zone_fam(2,3),'lineStyle','none','marker','o','markerSize',markerS,'color',colorBlack,'markerFacecolor',colorLightGray);
    hold on;
    plot(3,m_timeBlock_zone_fam(2,1),'lineStyle','none','marker','o','markerSize',markerS,'color',colorBlack,'markerFacecolor',colorLightGray);
    text(0,-4,['SR test: ',num2str(p_sr_stim(2),3)],'fontSize',fontM);
    ylabel('Time (min)','fontSize',fontM);
    
    set(hTimeStimBar,'Box','off','TickDir','out','XLim',[0.5 3.5],'XTick',[1 3],'XTickLabel',{'Stimulated','Unstimulated'},'fontSize',fontM);
    set(hTimeStimBar(1),'YLim',[0 7],'YTick',[0:2:8]);
    set(hTimeStimBar(2),'YLim',[0 7],'YTick',[0:2:8]);

print('-painters','-r300','-dtiff',['fig2_behavior_',datestr(now,formatOut),'_v3.tif']);
print('-painters','-r300','-depsc',['fig2_behavior_',datestr(now,formatOut),'_v3.ai']); % print from matlab2014a
close all;