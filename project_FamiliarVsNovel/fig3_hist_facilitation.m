...
% activated (all)
hPlot(1) = axes('Position',axpt(nCol,nRow,1:2,1,spaceFig,wideInterval));
%     for iLight = 1:4
...
%     end
    for iLight = 1:5
        lightPatch(iLight) = patch([200*(iLight-1) 200*(iLight-1)+80 200*(iLight-1)+80 200*(iLight-1)],[0 0 yLim(1) yLim(1)],colorLLightBlue,'LineStyle','none');
        hold on;
    end
...
% inhibited
hPlot(2) = axes('Position',axpt(nCol,nRow,1:2,2,spaceFig,wideInterval));
%     for iLight = 1:4
...
% in sensitive
hPlot(3) = axes('Position',axpt(nCol,nRow,1:2,3,spaceFig,wideInterval));
%     for iLight = 1:4
...
% activated (all)
hPlot(4) = axes('Position',axpt(nCol,nRow,1:2,4,spaceFig,wideInterval));
%     for iLight = 1:4
...
%     end
    for iLight = 1:5
        lightPatch(iLight) = patch([200*(iLight-1) 200*(iLight-1)+80 200*(iLight-1)+80 200*(iLight-1)],[0 0 yLim_bc(1) yLim_bc(1)],colorLLightBlue,'LineStyle','none');
        hold on;
    end
...
% no response
hPlot(5) = axes('Position',axpt(nCol,nRow,1:2,5,spaceFig,wideInterval));
%     for iLight = 1:4
...
%     end
    for iLight = 1:5
        lightPatch(iLight) = patch([200*(iLight-1) 200*(iLight-1)+80 200*(iLight-1)+80 200*(iLight-1)],[0 0 yLim_bc(2) yLim_bc(2)],colorLLightBlue,'LineStyle','none');
        hold on;
    end
...
set(hPlot,'Box','off','TickDir','out','XLim',[-20 950],'XTick',[-20,0:100:950],'fontSize',fontM);
set(hPlot(4),'YLim',[0 yLim_bc(1)],'YTick',[0:100:yLim_bc(1)]);
set(hPlot(5),'YLim',[0 yLim_bc(2)]);

% print('-painters','-r300','-dtiff',['fig3_PSTH_novel_facilitation_',datestr(now,formatOut),'.tif']);
print('-painters','-r300','-depsc',['fig3_PSTH_novel_facilitation_',datestr(now,formatOut),'.ai']);
close all;