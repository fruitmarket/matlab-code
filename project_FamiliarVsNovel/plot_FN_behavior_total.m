...
hGroupTotal = axes('Position',axpt(nCol,nRow,4,1,[],wideInterval));
    ...
    text(0.5,25,['PREvsPOST(nov): p=', num2str(p_Fri_nov(2,1),3)],'color',colorRed,'fontSize',fontM);
    text(0.5,22,['STIMvsPOST(nov): p=', num2str(p_Fri_nov(3,1),3)],'color',colorRed,'fontSize',fontM);
...
hGroupStim = axes('Position',axpt(nCol,nRow,4,2,[],wideInterval));
 hBar(1) = bar(xptBar_nov,m_timeBlock_stim_nov(2:4,3),'barWidth',barWidth,'lineStyle','-','edgeColor',colorBlack,'faceColor',colorDarkGray,'lineWidth',1);
    ...
    text(0.5,10,['PREvsPOST(nov): p=', num2str(p_Fri_nov(2,2),3)],'color',colorRed,'fontSize',fontM);
    text(5.3,6,'n.s.','color',colorRed,'fontSize',fontM);
    ylabel('Time (min)','fontSize',fontM);
...
hGroupUnstim = axes('Position',axpt(nCol,nRow,4,3,[],wideInterval));
    ...
    text(0.5,10,['STIMvsPOST(nov): p=', num2str(p_Fri_nov(3,3),3)],'color',colorRed,'fontSize',fontM);
    text(0.5,9,['STIMvsPOST(fam): p=', num2str(p_Fri_fam(3,3),3)],'color',colorRed,'fontSize',fontM);
    ylabel('Time (min)','fontSize',fontM);
    set(hGroupUnstim,'Box','off','TickDir','out','XLim',[0 8],'XTick',[2,6],'XTickLabel',{'Novel','Familiar'},'YLim',[0 10],'YTick',[0:2:10],'fontSize',fontM);

print('-painters','-r300','-dtiff',['plot_total_behavior_',datestr(now,formatOut),'.tif']);