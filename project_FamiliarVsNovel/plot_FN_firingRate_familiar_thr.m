...
m_inSTIM = cellfun(@(x) x(2), T.m_lapFrInzone);
m_inPOST = cellfun(@(x) x(3), T.m_lapFrInzone);

m_outPRE = cellfun(@(x) x(1), T.m_lapFrOutzone);
m_outSTIM = cellfun(@(x) x(2), T.m_lapFrOutzone);
m_outPOST = cellfun(@(x) x(3), T.m_lapFrOutzone);

m_totPRE = cellfun(@(x) x(1), T.m_lapFrTotalzone);
m_totSTIM = cellfun(@(x) x(2), T.m_lapFrTotalzone);
m_totPOST = cellfun(@(x) x(3), T.m_lapFrTotalzone);

%% threshold
% idx_dec = T.idxmFrIn == -1; % STM block decreased neurons (inactivation)
% min_lapFrInPRE = min(m_inPRE(PN & idx_dec));
% idx_thrPass = m_inPRE>=min_lapFrInPRE;
% 
% PN_act = PN & T.idxmFrIn == 1 & idx_thrPass;
% PN_inh = PN & T.idxmFrIn == -1 & idx_thrPass;
% PN_non = PN & T.idxmFrIn == 0 & idx_thrPass;
% IN_inh = IN & T.idxmFrIn == -1 & idx_thrPass;

min_lapFrInPRE = min(m_inPRE(PN & T.idxmSpkIn == -1)); % <<<<<<<<<<<<<< firing rate base
idx_thrPass = m_inPRE>=min_lapFrInPRE;

PN_act = PN & T.idxmSpkIn == 1 & idx_thrPass;
PN_inh = PN & T.idxmSpkIn == -1 & idx_thrPass;
PN_non = PN & T.idxmSpkIn == 0 & idx_thrPass;
IN_inh = IN & T.idxmSpkIn == -1 & idx_thrPass;
...
hold on;
errorbarJun(xptBar,mean_pn_in,sem_pn_in,eBarLength,eBarWidth,colorBlack);
text(1,5.8,['p = ',num2str(p_pn_in)],'fontSize',fontM);
ylabel('Firing rate (Hz)','fontSize',fontM);
...
hold on;
errorbarJun(xptBar,mean_pn_out,sem_pn_out,eBarLength,eBarWidth,colorBlack);
text(1,3.8,['p = ',num2str(p_pn_out)],'fontSize',fontM);
ylabel('Firing rate (Hz)','fontSize',fontM);
...
hold on;
errorbarJun(xptBar,mean_pn_tot,sem_pn_tot,eBarLength,eBarWidth,colorBlack);
text(1,3.8,['p = ',num2str(p_pn_tot)],'fontSize',fontM);
ylabel('Firing rate (Hz)','fontSize',fontM);
...
text(0.5,45,['n_all = ',num2str(n_in_inh)],'interpreter','none','fontSize',fontM);
text(3,45,['p = ',num2str(p_in_in)],'fontSize',fontM);
ylabel('Firing rate (Hz)','fontSize',fontM);
...
text(3,45,['p = ',num2str(p_in_out)],'fontSize',fontM);
ylabel('Firing rate (Hz)','fontSize',fontM);
...
text(3,45,['p = ',num2str(p_in_tot)],'fontSize',fontM);
ylabel('Firing rate (Hz)','fontSize',fontM);
...
set(hBar(4),'YLim',[0,6],'YTick',0:1:6);
set(hBar(5:6),'YLim',[0,4],'YTick',0:1:4);
set(hBar(7:9),'YLim',[0,50],'YTick',0:10:50);

print('-painters','-r300','-dtiff',['plot_firingRate_familiar_thr_',datestr(now,formatOut),'.tif']);