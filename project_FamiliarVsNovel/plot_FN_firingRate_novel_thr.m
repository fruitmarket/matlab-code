...
hold on;
errorbarJun(xptBar,mean_pn_in,sem_pn_in,eBarLength,eBarWidth,colorBlack);
text(1,4.8,['p = ',num2str(p_pn_in)],'fontSize',fontM);
ylabel('Firing rate (Hz)','fontSize',fontM);
...
hold on;
errorbarJun(xptBar,mean_pn_out,sem_pn_out,eBarLength,eBarWidth,colorBlack);
text(1,1.8,['p = ',num2str(p_pn_out)],'fontSize',fontM);
ylabel('Firing rate (Hz)','fontSize',fontM);
...
hold on;
errorbarJun(xptBar,mean_pn_tot,sem_pn_tot,eBarLength,eBarWidth,colorBlack);
text(1,1.8,['p = ',num2str(p_pn_tot)],'fontSize',fontM);
ylabel('Firing rate (Hz)','fontSize',fontM);
...
text(0.5,55,['n_all = ',num2str(n_in_inh)],'interpreter','none','fontSize',fontM);
...
set(hBar(4),'YLim',[0,5],'YTick',0:1:5);
set(hBar(5:6),'YLim',[0,2],'YTick',0:0.5:2);
set(hBar(7:9),'YLim',[0,60],'YTick',0:10:60);

print('-painters','-r300','-dtiff',['plot_firingRate_novel_thr_',datestr(now,formatOut),'.tif']);