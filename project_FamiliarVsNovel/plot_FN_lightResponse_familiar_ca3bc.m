...
PN = T.neuronType == 'PN';
tt_ca3bc = PN & ((T.mouseID == 'rbp005' & (T.tetrode == 'TT1' | T.tetrode == 'TT5')) | (T.mouseID == 'rbp006' & T.tetrode == 'T2') | (T.mouseID == 'rbp010' & T.tetrode == 'T6'));
...
m_lapFrInSTIM = cellfun(@(x) x(2),T.m_lapFrInzone); % PRE lap firing rate inzone
...
neuron_th_act(2) = false; % <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 
neuron_th_dir = PN & T.idxmFrIn == 1 & T.latencyTrack1st<10 & isnan(T.latencyTrack2nd) & idx_thrPass;
...
set(hPlot(4),'visible','off');
% for iLight = 1:4
%     lightPatch(iLight) = patch([20*(iLight-1) 20*(iLight-1)+10 20*(iLight-1)+10 20*(iLight-1)],[0 0 yLim(1) yLim(1)],colorLLightBlue,'LineStyle','none');
%     hold on;
% end
% hBar(4) = bar(xpt,m_peth_inh,'histc');
...
set(hPlot(5),'visible','off');
% for iLight = 1:4
%     lightPatch(iLight) = patch([20*(iLight-1) 20*(iLight-1)+10 20*(iLight-1)+10 20*(iLight-1)],[0 0 yLim(1) yLim(1)],colorLLightBlue,'LineStyle','none');
%     hold on;
% end
% hBar(5) = bar(xpt,m_peth_non,'histc');
...
set(hPlot(1),'YLim',[0 yLim(1)],'YTick',[0:40:yLim(1)]);
set(hPlot(2),'YLim',[0 yLim(2)],'YTick',[0:10:yLim(2)]);
set(hPlot(3),'YLim',[0 yLim(3)],'YTick',[0:40:yLim(3)]);
set(hPlot(4),'YLim',[0 yLim(4)],'YTick',[0:5:yLim(4)]);
set(hPlot(5),'YLim',[0 yLim(5)],'YTick',[0:5:yLim(5)]);

%% Threshold
% yLim = [120, 60, 100, 20, 15];
% 
...
% set(hScatter,'Box','off','TickDir','out','TickLength',[0.03 0.03],'XLim',[0,6],'XTick',[1,3,5],'XTickLabel',{'PRE','STIM','POST'},'fontSize',fontM);
% set(hPlot,'Box','off','TickDir','out','XLim',[-10 120],'fontSize',fontM);
% set(hPlot(1),'YLim',[0 yLim(1)],'YTick',[0:25:yLim(1)]);
% set(hPlot(2),'YLim',[0 yLim(2)],'YTick',[0:10:yLim(2)]);
...
cd(rtDir);
print('-painters','-r300','-dtiff',['plot_familiar_lightResponse_CA3bc_',datestr(now,formatOut),'.tif']);
close all;