...
% saveDir = 'E:\Dropbox\SNL\P4_FamiliarNovel\example_familiar\PN_act';
% cellID = T.cellID(PN & idx_dec & idx_thrPass);
% saveDir = 'E:\Dropbox\SNL\P4_FamiliarNovel\example_familiar\PN_inh';
% cellID = T.cellID(PN & ~(idx_inc | idx_dec) & idx_thrPass);
% saveDir = 'E:\Dropbox\SNL\P4_FamiliarNovel\example_familiar\PN_non';
...
%     fHandle = figure('PaperUnits','centimeters','PaperPosition',[0 0 5 5.5]);
    ...
    hPlot(2) = axes('Position',axpt(13,3,1:11,2,[0.2 0.04 0.85 0.85],[0.01 0.10]));
        plot([xptSpatial{1};xptSpatial{2};xptSpatial{3}],[yptSpatial{1};yptSpatial{2};yptSpatial{3}],'Marker','.','MarkerSize',markerS,'markerFaceColor',colorBlack,'markerEdgeColor','k','LineStyle','none');
