...
basePRE = cell2mat(T.rateMap1D_BasePRE(PN_th));
PRE = cell2mat(T.rateMap1D_PRE(PN_th));
STIM = cell2mat(T.rateMap1D_STM(PN_th));
POST = cell2mat(T.rateMap1D_POST(PN_th));
basePOST = cell2mat(T.rateMap1D_BasePOST(PN_th));

nBin = size(basePRE,2);

% normalize
basePRE = basePRE./repmat(max(basePRE,[],2),1,nBin);
PRE = PRE./repmat(max(PRE,[],2),1,nBin);
STIM = STIM./repmat(max(STIM,[],2),1,nBin);
POST = POST./repmat(max(POST,[],2),1,nBin);
basePOST = basePOST./repmat(max(basePOST,[],2),1,nBin);

% inzone
inZone_basePRE = basePRE(:,lightLoc_in);
inZone_PRE = PRE(:,lightLoc_in);
inZone_STIM = STIM(:,lightLoc_in);
inZone_POST = POST(:,lightLoc_in);
inZone_basePOST = basePOST(:,lightLoc_in);

% outzone
outZone_basePRE = basePRE(:,lightLoc_out);
outZone_PRE = PRE(:,lightLoc_out);
outZone_STIM = STIM(:,lightLoc_out);
outZone_POST = POST(:,lightLoc_out);
outZone_basePOST = basePOST(:,lightLoc_out);

[rCor_all_BASE, rCor_all_PRExSTIM, rCor_all_PRExPOST, rCor_all_STIMxPOST] = deal([]);
[rCor_in_BASE, rCor_in_PRExSTIM, rCor_in_PRExPOST, rCor_in_STIMxPOST] = deal([]);
[rCor_out_BASE, rCor_out_PRExSTIM, rCor_out_PRExPOST, rCor_out_STIMxPOST] = deal([]);

for iCell = 1:nPN_th
    rCor_all_BASE(iCell,1) = corr(basePRE(iCell,:)',basePOST(iCell,:)','rows','complete','type','pearson');
    ...
    rCor_in_BASE(iCell,1) = corr(inZone_basePRE(iCell,:)',inZone_basePOST(iCell,:)','rows','complete','type','pearson');
    ...
    rCor_out_BASE(iCell,1) = corr(outZone_basePRE(iCell,:)',outZone_basePOST(iCell,:)','rows','complete','type','pearson');
...
m_rCor_all_BASE = nanmean(rCor_all_BASE);
sem_rCor_all_BASE = nanstd(rCor_all_BASE,0,1)/sqrt(sum(double(~isnan(rCor_all_BASE))));
m_rCor_in_BASE = nanmean(rCor_in_BASE);
sem_rCor_in_BASE = nanstd(rCor_in_BASE,0,1)/sqrt(sum(double(~isnan(rCor_in_BASE))));
m_rCor_out_BASE = nanmean(rCor_out_BASE);
sem_rCor_out_BASE = nanstd(rCor_out_BASE,0,1)/sqrt(sum(double(~isnan(rCor_out_BASE))));
...
% basePRE vs basePOST
hCor(1) = axes('Position',axpt(nCol,nRow,1,1,[],wideInterval));
bar([1,2,3], [m_rCor_all_BASE, m_rCor_in_BASE, m_rCor_out_BASE], barWidth,'faceColor',colorLightGray);
hold on;
errorbarJun([1,2,3], [m_rCor_all_BASE, m_rCor_in_BASE, m_rCor_out_BASE], [sem_rCor_all_BASE, sem_rCor_in_BASE, sem_rCor_out_BASE],eBarLength, eBarWidth, colorBlack);
text(3.0,0.8,['n = ',num2str(nPN_th)],'fontSize',fontM);
ylabel('Spatial correlation (r)','fontSize',fontM);
title('basePRE vs basePOST [Familiar]','fontSize',fontM);

% PRExSTIM vs. PRExPOST
hCor(2) = axes('Position',axpt(3,1,1,1,axpt(nCol,nRow,1:2,2,[],wideInterval),wideInterval));
...
hCor(3) = axes('Position',axpt(3,1,2,1,axpt(nCol,nRow,1:2,2,[],wideInterval),wideInterval));
plot([-1,1],[-1,1],'lineStyle','--','color',colorDarkGray);
hold on;
...
hCor(4) = axes('Position',axpt(3,1,3,1,axpt(nCol,nRow,1:2,2,[],wideInterval),wideInterval));
plot([-1,1],[-1,1],'lineStyle','--','color',colorDarkGray);
hold on;
...
hCor(5) = axes('Position',axpt(3,1,2,1,axpt(nCol,nRow,1:2,3,[],wideInterval),wideInterval));
...
hCor(6) = axes('Position',axpt(3,1,3,1,axpt(nCol,nRow,1:2,3,[],wideInterval),wideInterval));
barIn = bar(xptBar,hist_out,'histc');
barIn.FaceColor = colorDarkGray;
xlabel('Distribution','fontSize',fontM);

hCor(7) = axes('Position',axpt(3,1,1,1,axpt(nCol,nRow,1:2,4,[],wideInterval),wideInterval));
...
set(hCor(1),'XLim',[0,4],'XTick',1:3,'XTickLabel',{'All','In-zone','Out-zone'},'YLim',[0,1],'YTick',0:0.2:1);
set(hCor(2:4),'XLim',[-1 1.1],'YLim',[-1 1.1],'XTick',-1:0.5:1,'YTick',-1:0.5:1);
set(hCor(5:6)','YLim',[0,25],'YTick',0:5:25);
set(hCor(7),'XLim',[0,9],'XTick',[1.5, 4.5, 7.5],'XTickLabel',{'PRExSTIM','PRExPOST','STIMxPOST'},'YLim',[-1,1.2],'YTick',-1:0.5:1);
set(hCor,'Box','off','TickDir','out','fontSize',fontM);

print('-painters','-r300','-dtiff',['plot_novel_spatialCorr_',datestr(now,formatOut),'.tif']);
close all;