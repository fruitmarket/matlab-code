...
m_inPRE = cellfun(@(x) x(1), T.m_lapFrInzone);
...
% n_th_ca3a = sum(double(PN_ca3a & idx_thrPass));
% n_th_ca3bc = sum(double(PN_ca3bc & idx_thrPass));
% n_th_total = n_th_ca3a + n_th_ca3bc;

% n_th_ca3a_act = sum(double(PN_ca3a & T.idxmFrIn == 1 & idx_thrPass));
% n_th_ca3a_inh = sum(double(PN_ca3a & T.idxmFrIn == -1 & idx_thrPass));
% n_th_ca3a_non = sum(double(PN_ca3a & T.idxmFrIn == 0 & idx_thrPass));
% 
% n_th_ca3bc_act = sum(double(PN_ca3bc & T.idxmFrIn == 1 & idx_thrPass));
% n_th_ca3bc_inh = sum(double(PN_ca3bc & T.idxmFrIn == -1 & idx_thrPass));
% n_th_ca3bc_non = sum(double(PN_ca3bc & T.idxmFrIn == 0 & idx_thrPass));

% prop_th_total = round([n_th_ca3a, n_th_ca3bc]/n_th_total*100)/100*100;
% prop_th_ca3a_total = round([n_th_ca3a_act, n_th_ca3a_inh, n_th_ca3a_non]/n_th_ca3a*100)/100*100;
% pro_th_ca3bc_total = round([n_th_ca3bc_act, n_th_ca3bc_inh, n_th_ca3bc_non]/n_th_ca3bc*100)/100*100;
...
prop_ca3a_total_th = round([n_ca3a_act_th, n_ca3a_inh_th, n_ca3a_no_th]/n_ca3a_th*100)/100*100;
...
peakFR_ca3a = cellfun(@(x) max(x(1:3)),T.peakFR1D_track(PN_ca3a));
peakFR_ca3bc = cellfun(@(x) max(x(1:3)),T.peakFR1D_track(PN_ca3bc));
...
sigTot_inc = sum(double(PN & cellfun(@(x) x(2,3)<0.05, T.p_ttestFr) & cellfun(@(x) x(3)>x(1),T.m_lapFrTotalzone))); % increased
sigTot_dec = sum(double(PN & cellfun(@(x) x(2,3)<0.05, T.p_ttestFr) & cellfun(@(x) x(3)<x(1),T.m_lapFrTotalzone))); % decreased
...
% In-zone 
sigIn_inc = PN & cellfun(@(x) x(2,1)<0.05, T.p_ttestFr) & cellfun(@(x) x(3)>x(1),T.m_lapFrInzone); % increased
sigIn_dec = PN & cellfun(@(x) x(2,1)<0.05, T.p_ttestFr) & cellfun(@(x) x(3)<x(1),T.m_lapFrInzone); % decreased
n_sigIn_inc = sum(double(sigIn_inc));
n_sigIn_dec = sum(double(sigIn_dec));

% Out-zone
sigOut_inc = PN & cellfun(@(x) x(2,2)<0.05, T.p_ttestFr) & cellfun(@(x) x(3)>x(1),T.m_lapFrOutzone); % increased
sigOut_dec = PN & cellfun(@(x) x(2,2)<0.05, T.p_ttestFr) & cellfun(@(x) x(3)<x(1),T.m_lapFrOutzone); % decreased
n_sigOut_inc = sum(double(sigOut_inc));
n_sigOut_dec = sum(double(sigOut_dec));

% Total
sigTot_inc = PN & cellfun(@(x) x(2,3)<0.05, T.p_ttestFr) & cellfun(@(x) x(3)>x(1),T.m_lapFrTotalzone); % increased
sigTot_dec = PN & cellfun(@(x) x(2,3)<0.05, T.p_ttestFr) & cellfun(@(x) x(3)<x(1),T.m_lapFrTotalzone); % increased
...
print('-painters','-r300','-dtiff',['plot_familiar_population_',datestr(now,formatOut),'.tif']);
close all;