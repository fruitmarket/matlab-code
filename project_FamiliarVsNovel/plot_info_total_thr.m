...
m_fam_inPRE = cellfun(@(x) x(1), T_fam.m_lapFrInzone);
...
hBar(1) = axes('Position',axpt(nCol,nRow,1,1,[],wideInterval));
    patch(xptBar_nov,[0 0 prop_n_nov_total_th(1) prop_n_nov_total_th(1)],colorLightRed,'LineStyle','-');
    hold on;
    patch(xptBar_nov,[prop_n_nov_total_th(1) prop_n_nov_total_th(1) sum(prop_n_nov_total_th) sum(prop_n_nov_total_th)],colorGray,'LineStyle','-');
    hold on;
    patch(xptBar_fam,[0 0 prop_n_fam_total_th(1) prop_n_fam_total_th(1)],colorLightRed,'LineStyle','-');
    hold on;
    patch(xptBar_fam,[prop_n_fam_total_th(1) prop_n_fam_total_th(1) sum(prop_n_fam_total_th) sum(prop_n_fam_total_th)],colorGray,'LineStyle','-');
    text(xptBar(1)*0.7,prop_n_nov_total_th(1)*0.5,'Act.','fontSize',fontM);
    text(xptBar(1)*0.7,prop_n_nov_total_th(1)+prop_n_nov_total_th(2)*0.5,'Oth.','fontSize',fontM);
    title('Total','fontSize',fontM);
    ylabel('Population ratio (%)','fontSize',fontM);

hBar(2) = axes('Position',axpt(nCol,nRow,2,1,[],wideInterval));
    patch(xptBar_nov,[0 0 prop_nov_ca3a_th(1) prop_nov_ca3a_th(1)],colorLightRed,'LineStyle','-');
    hold on;
    patch(xptBar_nov,[prop_nov_ca3a_th(1) prop_nov_ca3a_th(1) sum(prop_nov_ca3a_th) sum(prop_nov_ca3a_th)],colorGray,'LineStyle','-');
    hold on;
    patch(xptBar_fam,[0 0 prop_fam_ca3a_th(1) prop_fam_ca3a_th(1)],colorLightRed,'LineStyle','-');
    hold on;
    patch(xptBar_fam,[prop_fam_ca3a_th(1) prop_fam_ca3a_th(1) sum(prop_fam_ca3a_th) sum(prop_fam_ca3a_th)],colorGray,'LineStyle','-');
    text(xptBar(1)*0.7,prop_nov_ca3a_th(1)*0.5,'Act.','fontSize',fontM);
    text(xptBar(1)*0.7,prop_nov_ca3a_th(1)+prop_nov_ca3a_th(2)*0.5,'Oth.','fontSize',fontM);
    title('CA3a','fontSize',fontM);
    ylabel('Population ratio (%)','fontSize',fontM);

hBar(3) = axes('Position',axpt(nCol,nRow,3,1,[],wideInterval));
    patch(xptBar_nov,[0 0 prop_nov_ca3bc_th(1) prop_nov_ca3bc_th(1)],colorLightRed,'LineStyle','-');
    hold on;
    patch(xptBar_nov,[prop_nov_ca3bc_th(1) prop_nov_ca3bc_th(1) sum(prop_nov_ca3bc_th) sum(prop_nov_ca3bc_th)],colorGray,'LineStyle','-');
    hold on;
    patch(xptBar_fam,[0 0 prop_fam_ca3bc_th(1) prop_fam_ca3bc_th(1)],colorLightRed,'LineStyle','-');
    hold on;
    patch(xptBar_fam,[prop_fam_ca3bc_th(1) prop_fam_ca3bc_th(1) sum(prop_fam_ca3bc_th) sum(prop_fam_ca3bc_th)],colorGray,'LineStyle','-');
    text(xptBar(1)*0.7,prop_nov_ca3bc_th(1)*0.5,'Act.','fontSize',fontM);
...
hBar(4) = axes('Position',axpt(nCol,nRow,1,2,[],wideInterval));
    ...
    patch(xptBar_nov,[prop_nov_ca3a_th(1) prop_nov_ca3a_th(1) sum(prop_nov_ca3a_th) sum(prop_nov_ca3a_th)],colorGray,'LineStyle','-');
    hold on;
    patch(xptBar_fam,[0 0 prop_nov_ca3bc_th(1) prop_nov_ca3bc_th(1)],colorLightRed,'LineStyle','-');
    hold on;
    patch(xptBar_fam,[prop_nov_ca3bc_th(1) prop_nov_ca3bc_th(1) sum(prop_nov_ca3bc_th) sum(prop_nov_ca3bc_th)],colorGray,'LineStyle','-');
    text(xptBar(1)*0.7,prop_nov_ca3a_th(1)*0.5,'Act.','fontSize',fontM);
    text(xptBar(1)*0.7,prop_nov_ca3a_th(1)+prop_nov_ca3a_th(2)*0.5,'Oth.','fontSize',fontM);
...
hBar(5) = axes('Position',axpt(nCol,nRow,2,2,[],wideInterval));
    patch(xptBar_nov,[0 0 prop_fam_ca3a_th(1) prop_fam_ca3a_th(1)],colorLightRed,'LineStyle','-');
    hold on;
    patch(xptBar_nov,[prop_fam_ca3a_th(1) prop_fam_ca3a_th(1) sum(prop_fam_ca3a_th) sum(prop_fam_ca3a_th)],colorGray,'LineStyle','-');
    hold on;
    patch(xptBar_fam,[0 0 prop_fam_ca3bc_th(1) prop_fam_ca3bc_th(1)],colorLightRed,'LineStyle','-');
    hold on;
    patch(xptBar_fam,[prop_fam_ca3bc_th(1) prop_fam_ca3bc_th(1) sum(prop_fam_ca3bc_th) sum(prop_fam_ca3bc_th)],colorGray,'LineStyle','-');
    text(xptBar(1)*0.7,prop_fam_ca3a_th(1)*0.5,'Act.','fontSize',fontM);
    text(xptBar(1)*0.7,prop_fam_ca3a_th(1)+prop_fam_ca3a_th(2)*0.5,'Oth.','fontSize',fontM);
...
print('-painters','-r300','-dtiff',['plot_infoPopul_total_thr_',datestr(now,formatOut),'.tif']);