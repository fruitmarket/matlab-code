function [xpt,ypt,spikeBin,spikeRaw,spikeConv,spikeConvZ] = proj_fn_spatialrasterPETH(spikePosition, trialIndex, occupancy, win, binSize, resolution, dot)
...
%     spikeconv_temp = conv(spikehistOccu_temp,fspecial('Gaussian',[1 5*resolution],resolution),'same');
    spikeRaw(iCue,:) = spikehistOccu_temp;
    spikehist_temp_conv = conv(spikehist_temp(1:end-1),fspecial('Gaussian',[1 5*resolution],resolution),'same');
    occupancy_conv = conv(occupancy(iCue,:),fspecial('Gaussian',[1 5*resolution],resolution),'same');
    spikeConv(iCue,:) = spikehist_temp_conv./occupancy_conv;
end
