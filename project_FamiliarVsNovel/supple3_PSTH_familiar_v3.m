...
ca3bc_inh = PN_ca3bc & T.idxLight == -1;
...
hPlot(2) = axes('Position',axpt(nCol,nRow,1,2,spaceFig,wideInterval));
    ...
    text(90,yLim(2)*0.8,['n = ',num2str(n_ca3a_inh)],'fontSize',fontM);
    xlabel('Time (ms)','fontSize',fontM);
    ylabel('Rate (Hz)','fontSize',fontM);
...
hPlot(3) = axes('Position',axpt(nCol,nRow,1,3,spaceFig,wideInterval));
    ...
    text(90,yLim(3)*0.8,['n = ',num2str(n_ca3a_non)],'fontSize',fontM);
    xlabel('Time (ms)','fontSize',fontM);
    ylabel('Rate (Hz)','fontSize',fontM);

set(hPlot(1),'YLim',[0 yLim(1)],'YTick',[0:30:yLim(1)]);
set(hPlot(2),'YLim',[0 yLim(2)],'YTick',[0,yLim(2)]);
...
hPlot(5) = axes('Position',axpt(nCol,nRow,2,2,spaceFig,wideInterval));
    for iLight = 1:4
    ...
    hBar(5) = bar(xpt,m_ca3bc_inh,'histc');
    errorbarJun(xpt+1,m_ca3bc_inh,sem_ca3bc_inh,1,eBarWidth,colorDarkGray);
    text(90,yLim_bc(2)*0.8,['n = ',num2str(n_ca3bc_inh)],'fontSize',fontM);
    xlabel('Time (ms)','fontSize',fontM);
    ylabel('Rate (Hz)','fontSize',fontM);

hPlot(6) = axes('Position',axpt(nCol,nRow,2,3,spaceFig,wideInterval));
    for iLight = 1:4
        lightPatch(iLight) = patch([20*(iLight-1) 20*(iLight-1)+10 20*(iLight-1)+10 20*(iLight-1)],[0 0 yLim_bc(2) yLim_bc(2)],colorLLightBlue,'LineStyle','none');
        hold on;
    end
    hBar(6) = bar(xpt,m_ca3bc_non,'histc');
    errorbarJun(xpt+1,m_ca3bc_non,sem_ca3bc_non,1,eBarWidth,colorDarkGray);
    text(90,yLim_bc(2)*0.8,['n = ',num2str(n_ca3bc_non)],'fontSize',fontM);
    xlabel('Time (ms)','fontSize',fontM);
    ylabel('Rate (Hz)','fontSize',fontM);
...
set(hPlot,'Box','off','TickDir','out','XLim',[-20 100],'XTick',[-20:20:100],'fontSize',fontM);
set(hPlot(4),'YLim',[0 yLim_bc(1)],'YTick',[0:50:yLim_bc(1)]);
set(hPlot(5),'YLim',[0 yLim_bc(2)],'YTick',[0,yLim_bc(2)]);
set(hPlot(6),'YLim',[0 yLim_bc(2)],'YTick',[0,yLim_bc(2)]);

print('-painters','-r300','-dtiff',['supple3_PSTH_familiar_v3_',datestr(now,formatOut),'.tif']);
print('-painters','-r300','-depsc',['supple3_PSTH_familiar_v3_',datestr(now,formatOut),'.ai']);
close all;