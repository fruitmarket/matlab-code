...
% CA3 total
PN_th = PN & cellfun(@(x) max(max(x)), T.pethconvSpatial)>cri_peakFR & T.idxmSpkIn == 1;
...
% CA3 total
PN_th = PN & cellfun(@(x) max(max(x(2:4,:))), T.pethconvSpatial)>cri_peakFR & T.idxmSpkIn == 1;
...
% statistic test
p_val_nov(1) = ranksum(rCor_nov_in_PRExSTIM(~isnan(rCor_nov_in_PRExSTIM)), rCor_nov_out_PRExSTIM(~isnan(rCor_nov_out_PRExSTIM)));
p_val_nov(2) = ranksum(rCor_nov_in_PRExPOST(~isnan(rCor_nov_in_PRExPOST)), rCor_nov_out_PRExPOST(~isnan(rCor_nov_out_PRExPOST)));
p_val_nov(3) = ranksum(rCor_nov_in_STIMxPOST(~isnan(rCor_nov_in_STIMxPOST)), rCor_nov_out_STIMxPOST(~isnan(rCor_nov_out_STIMxPOST)));

p_val_fam(1) = ranksum(rCor_fam_in_PRExSTIM(~isnan(rCor_fam_in_PRExSTIM)), rCor_fam_out_PRExSTIM(~isnan(rCor_fam_out_PRExSTIM)));
p_val_fam(2) = ranksum(rCor_fam_in_PRExPOST(~isnan(rCor_fam_in_PRExPOST)), rCor_fam_out_PRExPOST(~isnan(rCor_fam_out_PRExPOST)));
p_val_fam(3) = ranksum(rCor_fam_in_STIMxPOST(~isnan(rCor_fam_in_STIMxPOST)), rCor_fam_out_STIMxPOST(~isnan(rCor_fam_out_STIMxPOST)));
...
hCor(1) = axes('Position',axpt(nCol,nRow,1,1,figSize,wideInterval));
    ...
    text(0,-1.5,['ranksum: ',num2str(p_val_nov,3)],'fontSize',fontM);
...
hCor(2) = axes('Position',axpt(nCol,nRow,2,1,figSize,wideInterval));
    ...
    text(0,-1.5,['ranksum: ',num2str(p_val_fam,3)],'fontSize',fontM);
...
print('-painters','-r300','-dtiff',['supple5_spatialcorrelation_',datestr(now,formatOut),'.tif']);
print('-painters','-r300','-depsc',['supple5_spatialcorrelation_',datestr(now,formatOut),'.ai']);
