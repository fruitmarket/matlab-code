...
ca3_inh = PN & T.idxmSpkIn == -1;
ca3_non = PN & T.idxmSpkIn == 0;

basePRE = T.rateMap1D_BasePRE;
basePOST = T.rateMap1D_BasePOST;
corr = cell2mat(cellfun(@(x,y) corr(x',y'),basePRE,basePOST,'UniformOutput',0));

corr_act = corr(ca3_act & ~isnan(corr));
corr_inh = corr(ca3_inh & ~isnan(corr));
corr_non = corr(ca3_non & ~isnan(corr));

n_corr = [length(corr_act), length(corr_inh), length(corr_non)];
mean_corr = [nanmean(corr_act), nanmean(corr_inh), nanmean(corr_non)];
sem_corr = [nanstd(corr_act,0,1)/sqrt(n_corr(1)), nanstd(corr_inh,0,1)/sqrt(n_corr(2)), nanstd(corr_non,0,1)/sqrt(n_corr(3))];
group = [repmat('act',[n_corr(1),1]);repmat('inh',[n_corr(2),1]);repmat('non',[n_corr(3),1])];

p = kruskalwallis([corr_act; corr_inh; corr_non],group,'off');
